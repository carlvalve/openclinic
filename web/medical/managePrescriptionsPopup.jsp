    ...
    private StringBuffer objectsToHtml(Vector objects, String sWebLanguage, User activeUser){
        ...
        for(int i=0; i<objects.size(); i++){
			...
			if(activeEncounter!=null){
				...
				if(productDeliveries.get(encounteruid+"."+prescr.getProductUid())==null){
					double totalDeliveredQuantity = prescr.getDeliveredQuantity();
					double expiredDeliveredQuantity = prescr.getExpiredDeliveredQuantity();
					System.out.println("totaldelivered="+totalDeliveredQuantity);
					System.out.println("expired="+expiredDeliveredQuantity);
					System.out.println("product="+prescr.getProductUid());
					System.out.println("begin="+activeEncounter.getBegin());
					System.out.println("encounteruid="+activeEncounter.getUid());
					productDeliveries.put(encounteruid+"."+prescr.getProductUid(),totalDeliveredQuantity-expiredDeliveredQuantity);
				}
				deliveredQuantity=(Double)productDeliveries.get(encounteruid+"."+prescr.getProductUid());
			}
			...
			if(activeEncounter!=null){
	            productDeliveries.put(encounteruid+"."+prescr.getProductUid(),deliveredQuantity);
			}
...
<table width="100%" cellspacing="0" cellpadding="0" class="sortable" id="searchresults">
    ...
    <tr>
        ...
        <td class="admin"><%=getTran(request,"Web","authorizedreceiver",sWebLanguage)%></td>
...
function doSave(){
  ...
  calculatePackagesNeeded();
  <%
  	if((sEditPrescrUid.length()==0 || sEditPrescrUid.equalsIgnoreCase("-1")) && MedwanQuery.getInstance().getConfigInt("enablePrescriptionSelectionOfDispensingStock",0)==1){%>
...
function selectDispensingStock(){
	if(document.getElementById('EditProductUid').value.length>0){
		openPopup("/medical/selectDispensingStock.jsp&EditProductUid="+document.getElementById('EditProductUid').value+"&EditRequiredPackages="+document.getElementById('EditRequiredPackages').value);
	}
	else{
		doSavePart2();
	}
