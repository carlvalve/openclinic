...
<%
    ...
    while (iter.hasNext()) {
        ...
        sbResults.append("<tr class='list" + sClass + "'>" +
                "<td><img src=\""+sCONTEXTPATH + "/_img/icons/icon_delete.png\" class=\"link\" alt='" + getTranNoLink("Web", "delete", sWebLanguage) + "' onclick=\"doDelete('" + sCode + "','" + sCodeType + "');\"></td>" +
                "<td>" + sCode + "</td>" +
...
<form name="DeleteDiagnosisForm" method="post" action="<c:url value='/main.do'/>?Page=medical/manageServiceDiagnoses.jsp&ts=<%=getTs()%>">
    ...
    <img src="<c:url value="/_img/icons/icon_search.png"/>" class="link" alt="<%=getTranNoLink("Web","select",sWebLanguage)%>" onclick="searchService('EditService','EditServiceName');">
    <img src="<c:url value="/_img/icons/icon_delete.png"/>" class="link" alt="<%=getTranNoLink("Web","clear",sWebLanguage)%>" onclick="DeleteDiagnosisForm.EditService.value='';DeleteDiagnosisForm.EditServiceName.value='';">
