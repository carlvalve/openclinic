...
<form name="stats">
	<%
		out.print(ScreenHelper.writeTblHeader(getTran(request,"Web","select.datasource",sWebLanguage),sCONTEXTPATH));
	%>
		<tr>
			<td width="1%">
				<input class="radiobutton" type="radio" name="datasource" checked/><%=getTran(request,"web","hospital",sWebLanguage)%>&nbsp;
			</td>
			<td>
		...
		<tr>
			<td>
				<input class="radiobutton" type="radio" name="datasource"/><%=getTran(request,"web","district",sWebLanguage)%>&nbsp;
			</td>
			<td>
			<%
...
<%
    if(activeUser.getAccessRight("statistics.globalpathologydistribution.select")){
        out.print(ScreenHelper.writeTblHeader(getTran(request,"Web","statistics.pathologystats",sWebLanguage),sCONTEXTPATH)
            ...
            +writeTblChildNoButton("main.do?Page=datacenterstatistics/mortality.jsp",getTran(request,"Web","statistics.mortality",sWebLanguage))
    ...
    if(activeUser.getAccessRight("statistics.select")) {
        ...
        out.print(ScreenHelper.writeTblHeader(getTran(request,"Web","statistics.insurancestats",sWebLanguage),sCONTEXTPATH)
    ...
    if(activeUser.getAccessRight("statistics.chin.select")){
        ...
        out.print(ScreenHelper.writeTblHeader(getTran(request,"Web","chin",sWebLanguage),sCONTEXTPATH)
                ...
                +writeTblChildWithCode("javascript:minisanteReport()",getTran(request,"Web","chin.minisantereport",sWebLanguage))
