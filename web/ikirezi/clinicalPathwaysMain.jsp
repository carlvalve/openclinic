...
<%
	if(request.getParameter("initPatient")!=null){
		...
		long year=365*day;
		activePatient.dateOfBirth=ScreenHelper.formatDate(new java.util.Date(new java.util.Date().getTime()-age*year));
...
<%
	if(activePatient!=null){
		...
		while(eSigns.hasMoreElements()){
			...
			System.out.println("key="+key+" - value="+value.split(";")[0]);
		}
		...
		for(int n=0;n<pathwayFiles.length;n++){
			...
			while(pathways.hasNext()){
				...
				if(isPathwayApplicable(pathway,signs)){
					...
					if(bCanceled){
						out.println("<tr><td colspan='2' class='admin2'><b><font style='color: grey'>"+getTran(request,"spt","excluded",sWebLanguage)+"</font></b></td></tr>");
						continue;
					}
					if(bSignsToExclude){
