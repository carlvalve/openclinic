...
<%
	...
	String sptPointer = Pointer.getPointer("activespt."+activePatient.personid);
	...
	if(sptSigns==null || request.getParameter("resetButton")!=null){
		...
		if(sptPointer.length()>0){
			out.println("<script>");
			out.println("if(confirm('"+getTranNoLink("web","loadlastspt",sWebLanguage)+"')){window.location.href='main.do?Page=ikirezi/clinicalPathways.jsp&getLastSpt=1';}");
			out.println("</script>");
			out.flush();
		}
	}
	if(request.getParameter("getLastSpt")!=null && request.getParameter("resetButton")==null){
		if(sptPointer.length()>0){
	...
	if(activeEncounter!=null){
		ikireziSigns = Ikirezi.getEncounterSymptoms(activeEncounter.getUid());
	}
					...
					while(excludes.hasNext()){
						...
						if(sptSigns.get(excludeSpt)==null){
							...
							if(checkString(exclude.attributeValue("minage")).length()>0){
								if(activePatient.getAgeInMonths()<Integer.parseInt(exclude.attributeValue("minage"))){
									bMatch=false;
								}
							}
							if(checkString(exclude.attributeValue("maxage")).length()>0){
								if(activePatient.getAgeInMonths()>Integer.parseInt(exclude.attributeValue("maxage"))){
									bMatch=false;
								}
							}
							if(checkString(exclude.attributeValue("gender")).length()>0){
								if(!exclude.attributeValue("gender").toUpperCase().contains(activePatient.gender.toUpperCase())){
									bMatch=false;
								}
							}
							if(bMatch){
