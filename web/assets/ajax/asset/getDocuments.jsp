	...
	<%
		...
		for(int n=0;n<docs.size();n++){
			...
			if(doc!=null && ScreenHelper.checkString(doc.storageName).length()>0){
				...
				out.println("<tr><td width='1%' nowrap>"+(!bLocked && activeUser.getAccessRight("assets.edit")?"<img src='"+sCONTEXTPATH+"/_img/icons/icon_delete.gif' onclick='deleteDocument("+doc.getUid().split("\\.")[1]+")'/> ":"")+extension+"</td><td>"+link+"</td></tr>");
			}
		}
	%>
</table>