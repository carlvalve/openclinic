...
<%!
    //--- SAVE DIAGNOSES TABLE --------------------------------------------------------------------
    private void saveDiagnosesToTable(Hashtable ICPCCodes, Hashtable ICD10Codes, String sTransactionUID,
    		                          SessionContainerWO sessionContainerWO, Encounter encounter){
            ...
            if(code.indexOf("ICPCCode") == 0){
                if(ScreenHelper.checkString((String)ICPCCodes.get("Gravity"+code)).length() > 0 && ScreenHelper.checkString((String) ICPCCodes.get("Certainty"+code)).length() > 0) {
                    Diagnosis.saveTransactionDiagnosisWithServiceAndFlags(code,
	                            ...
	                            "ICPCCode",
	                            "icpc",
	                            new java.sql.Timestamp(encounter.getBegin().getTime()),
	                            sTransactionUID,
...
<%	
    boolean isFindPatient   = checkString(request.getParameter("findPatient")).length() > 0,
            isNewPatient    = checkString(request.getParameter("newPatient")).length() > 0,
            isNewEncounter  = checkString(request.getParameter("newEncounter")).length() > 0,
            isSaveEncounter = checkString(request.getParameter("saveEncounter")).length() > 0,
            isFindEncounter = checkString(request.getParameter("findEncounter")).length() > 0;
    
    String sPatientLastname  = checkString(request.getParameter("patientLastname")),
           sPatientFirstname = checkString(request.getParameter("patientFirstname")),
           sPatientDateOfBirth = checkString(request.getParameter("patientDateOfBirth")),
           sPatientGender    = checkString(request.getParameter("patientGender")),
           sPatientUID       = checkString(request.getParameter("PatientUID")),
           sActivePatientUID = checkString(request.getParameter("activePatientUID"));
    ...
    String sEditEncounterUID     = checkString(request.getParameter("EditEncounterUID")),
           ...
           sEditEncounterService = checkString(request.getParameter("EditEncounterService")),
           sEditEncounterServiceName = checkString(request.getParameter("EditEncounterServiceName")),
           sEditEncounterOutcome = checkString(request.getParameter("EditEncounterOutcome")),
           sEditEncounterOrigin  = checkString(request.getParameter("EditEncounterOrigin"));
...
<table class='list' border='0' width='100%' cellspacing='1'>
    <tr>
        ...
        <td class="admin"><%=HTMLEntities.htmlentities(getTran(request,"web","firstname",sWebLanguage))%></td>
        <td class="admin2"><input type="text" class="text" name="patientFirstname" id="pf" value="<%=sPatientFirstname%>" size="15"/></td>
        <td class="admin"><%=getTran(request,"web","dateofbirth",sWebLanguage)%></td>
        <td class="admin2"><input type="text" class="text" name="patientDateOfBirth" id="pd" value="<%=sPatientDateOfBirth%>" size="12" maxlength="10" OnBlur='checkDate(this)'/></td>
        ...
        <%-- BUTTONS --%>
        <td class="admin2">
            <input type="submit" class="button" class="text" name="findPatient" value="<%=getTranNoLink("web","find",sWebLanguage)%>" size="1"/>
            <input name="newPatient" class="button" type="hidden"/>
            <input type="button" class="button" class="text" name="newPatientButton" value="<%=getTranNoLink("web","new",sWebLanguage)%>" size="1" onclick="newpat();"/>
            <input type="button" class="button" class="text" name="clear" value="<%=getTranNoLink("web","clear",sWebLanguage)%>" size="1" onclick="clearPatient();EditPatientForm.document.getElementById('patientLastname').focus();"/>
        </td>
    </tr>
</table>
...
<%
    ...
    if(isFindPatient || isFindEncounter) {
        ...
        if(!isNewPatient && (sPatientLastname.trim().length() > 0 || sPatientFirstname.trim().length() > 0 || sPatientDateOfBirth.trim().length() > 0 || sPatientUID.trim().length() > 0)) {
            ...
            if(persons.size() > 1) {
%>
<table width="100%" class="list" border="0" cellpadding="0" cellspacing="1">
    <%
...
<%=persons.size()%> <%=getTran(request,"web","recordsFound",sWebLanguage)%>
<%
}
...
<table width="100%" class="list" border="0" cellpadding="0" cellspacing="1">
    <tr>
        <td class="admin2"><b><%=activePerson.personid%></b></td>
        <td class="admin2"><b><%=HTMLEntities.htmlentities(activePerson.lastname+" "+activePerson.firstname+"  °"+activePerson.dateOfBirth+" "+activePerson.gender)%></b></td>
        <td class="admin2"><input type="submit" class="text button" name="newEncounter" value="<%=getTranNoLink("web","newencounter",sWebLanguage)%>" size="1"/></td>
    </tr>
</table>
<%
...
<table width="100%" class="list" border="0" cellpadding="0" cellspacing="1">
    ...
    <%-- date begin --%>
    <tr>
        <td class="admin"><%=HTMLEntities.htmlentities(getTran(request,"Web","begindate",sWebLanguage))%></td>
        <td class="admin2">
    ...
    <%-- origin --%>
    <tr>
        <td class="admin" width="<%=sTDAdminWidth%>"><%=getTran(request,"openclinic.chuk","urgency.origin",sWebLanguage)%></td>
        <td class="admin2">
    ...
    <tr id="Service">
        ...
        <td class='admin2'>
            ...
            <img src="<c:url value="/_img/icons/icon_search.png"/>" class="link" alt="<%=getTranNoLink("Web","select",sWebLanguage)%>" onclick="searchService('EditEncounterService','EditEncounterServiceName');">
            <img src="<c:url value="/_img/icons/icon_delete.png"/>" class="link" alt="<%=getTranNoLink("Web","clear",sWebLanguage)%>" onclick="EditPatientForm.EditEncounterService.value='';EditPatientForm.EditEncounterServiceName.value='';">
        </td>
    </tr>
	...
	<%
	    String sRfe = ReasonForEncounter.getReasonsForEncounterAsHtml(sEditEncounterUID,sWebLanguage,"_img/icons/icon_delete.png","deleteRFE($serverid,$objectid)");
	%>
    <tr class="admin">
        <td align="left" colspan="2"><a href="javascript:openPopup('healthrecord/findRFE.jsp&field=rfe&patientuid=<%=sPatientUID%>&encounterUid=<%=sEditEncounterUID%>&ts=<%=getTs()%>',700,400)"><%=getTran(request,"openclinic.chuk","rfe",sWebLanguage)%> <%=getTran(request,"Web.Occup","ICPC-2",sWebLanguage)%>/<%=getTran(request,"Web.Occup","ICD-10",sWebLanguage)%></a></td>
    </tr>
    <tr>
    ...
    <tr class="admin">
        <td align="left" colspan="2">
	        <a href="javascript:openPopup('healthrecord/findICPC.jsp&ts=<%=getTs()%>&patientuid=<%=sPatientUID%>&showpatientencounters=1',700,400);void(0);"><%=
	            getTran(request,"openclinic.chuk","diagnostic",sWebLanguage)%> <%=getTran(request,"Web.Occup", "ICPC-2", sWebLanguage)%>/<%=getTran(request,"Web.Occup", "ICD-10", sWebLanguage)%>
	        </a>
        </td>
    </tr>
    <tr>
...
<%=ScreenHelper.setFormButtonsStart()%>
	<input class='button' type="button" name="saveMe" value='<%=getTranNoLink("Web","save",sWebLanguage)%>' onclick='doSubmit();'>&nbsp;
...
<tr>
    ...
    <td class="admin2"><b><%=(encounter.getService()==null?"":encounter.getService().getLabel(sWebLanguage))+(encounter.getOutcome()==null?"":" ("+getTran(request,"encounter.outcome",encounter.getOutcome(),sWebLanguage)+")")%></b></td>
</tr>
                <%
            }
        }
...
<script>
  ...
  function doSubmit(){
    var begin = createDate(document.getElementsByName("EditEncounterBegin")[0].value),
        end   = createDate(document.getElementsByName("EditEncounterEnd")[0].value);
