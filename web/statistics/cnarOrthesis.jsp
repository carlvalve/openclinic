...
<%
	...
	String sQuery="select count(*) total from oc_debets d,oc_encounters e, oc_prestations p where p.oc_prestation_objectid=replace(d.oc_debet_prestationuid,'"+MedwanQuery.getInstance().getConfigString("serverId")+".','') and e.oc_encounter_objectid=replace(d.oc_debet_encounteruid,'"+MedwanQuery.getInstance().getConfigString("serverId")+".','') and oc_prestation_class='"+MedwanQuery.getInstance().getConfigString("cnarOrthopedicEquipment","orth")+"' and d.oc_debet_date>=? and d.oc_debet_date<=?";
	...
	%>
	<table width="100%">
		<tr class='admin'><td colspan='2'><%=getTran(request,"cnar","statistics.ortho.title1",sWebLanguage)%></td></tr>
		<tr>
...
%>
	<br/><hr/><br/>
	<table width="100%">
		<tr class='admin'><td colspan='3'><%=getTran(request,"cnar","statistics.ortho.title2",sWebLanguage)%></td></tr>
		<tr class='admin'>
...
%>
	<br/><hr/><br/>
	<table width="100%">
		<tr class='admin'><td colspan='6'><%=getTran(request,"cnar","statistics.ortho.title3",sWebLanguage)%></td></tr>
		<tr class='admin'>
			<td rowspan='2'><%=getTran(request,"cnar","statistics.prestation.name",sWebLanguage)%></td>
...
%>
	<br/><hr/><br/>
	<table width="100%">
		<tr class='admin'><td colspan='8'><%=getTran(request,"cnar","statistics.ortho.title4",sWebLanguage)%></td></tr>
		<tr class='admin'>
			<td rowspan='2'><%=getTran(request,"cnar","statistics.prestation.name",sWebLanguage)%></td>
...
<%
	sQuery="select count(*) total from oc_debets d,oc_encounters e, oc_prestations p where p.oc_prestation_objectid=replace(d.oc_debet_prestationuid,'"+MedwanQuery.getInstance().getConfigString("serverId")+".','') and e.oc_encounter_objectid=replace(d.oc_debet_encounteruid,'"+MedwanQuery.getInstance().getConfigString("serverId")+".','') and oc_prestation_class='"+MedwanQuery.getInstance().getConfigString("cnarRepair","rep")+"' and d.oc_debet_date>=? and d.oc_debet_date<=?";
	ps = conn.prepareStatement(sQuery);
	ps.setTimestamp(1,new java.sql.Timestamp(dBegin.getTime()));
	ps.setTimestamp(2,new java.sql.Timestamp(dEnd.getTime()));
	rs = ps.executeQuery();
...
%>
	<br/><hr/><br/>
	<table width="100%">
		<tr class='admin'><td colspan='2'><%=getTran(request,"cnar","statistics.ortho.title5",sWebLanguage)%></td></tr>
		<tr>
			<td class='admin'><%= getTran(request,"web","total",sWebLanguage)%></td>
			<td class='admin'><%= total %></td>
		</tr>
