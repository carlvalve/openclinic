...
<%
    if (isSaveEncounter) {
        ...
        try {
            ...
            saveDiagnosesToTable(RequestParameterParser.getInstance().parseRequestParameters(request, "ICPCCode"), RequestParameterParser.getInstance().parseRequestParameters(request, "ICD10Code"), RequestParameterParser.getInstance().parseRequestParameters(request, "DSM4Code"), "", sessionContainerWO, encounter);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
...
<table width="100%" class="list" border="0">
    ...
    <tr>
        <td class="admin2"><b>
            <a href="<c:url value='/'/>popup.jsp?Page=statistics/quickFileDSM4.jsp&PopupHeight=600&PopupWidth=800&findPatient=1&PatientUID=<%=activePerson.personid%>"><%=
...
<table class='list' border='0' width='100%' cellspacing='1'>
    ...
    <tr id="Service">
        ...
        <td class='admin2'>
            ...
            <img src="<c:url value="/_img/icons/icon_search.png"/>" class="link"
                 alt="<%=getTranNoLink("Web","select",sWebLanguage)%>"
                 onclick="searchService('EditEncounterService','EditEncounterServiceName');">
            <img src="<c:url value="/_img/icons/icon_delete.png"/>" class="link"
    ...
    <tr>
        <td id='dsm4codes' colspan="2" align="left"><%=HTMLEntities.htmlentities(sDSM4Html)%></td>
    </tr>
</table>
...
<table width="100%" class="list" border="0">
    ...
    <tr>
        <td class="admin2"><b><a
                href="<c:url value='/'/>popup.jsp?Page=statistics/quickFileDSM4.jsp&PopupHeight=600&PopupWidth=800&EditEncounterUID=<%=encounter.getUid()%>&PatientUID=<%=activePerson.personid%>&findEncounter=1"/><%=
