...
<%
	...
	if(productStock!=null){
		...
		if(product!=null){
			sProductCategory=MedwanQuery.getInstance().getConfigString("cntsCategoryToProductConversion."+product.getProductSubGroup().toLowerCase(),"");
	...
	for(int n=0;n<bloodgifts.size();n++){
		...
		if(!bRejected && MedwanQuery.getInstance().getConfigString("cntsBloodProducsNeedingABOCompatibility", "*cgr*st*cp*pfc*prp*").contains("*"+sProductCategory.toLowerCase()+"*")){
		...
		if(!bRejected){
			try{
				//now we check if the requested bloodproducttype has been made available
				Vector bloodlabs = MedwanQuery.getInstance().getTransactionsByType(Integer.parseInt(sPatientUid), "be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_CNTSLAB_RECORD");
				for(int k=0;k<bloodlabs.size();k++){
					TransactionVO bloodlab = (TransactionVO)bloodlabs.elementAt(k);
					String expirydate=bloodlab.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNTSLAB_"+sProductCategory.toUpperCase()+"EXPIRYDATE");
					java.util.Date expdate = ScreenHelper.parseDate(expirydate);
					if(expdate.after(new java.util.Date())){
						nPockets=0;
						try{
							nPockets=Integer.parseInt(bloodlab.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNTSLAB_"+sProductCategory.toUpperCase()+"POCKETS"));
						}
						catch(Exception r){}
						...
						if(nPockets>0){
							if(!sRefABO.equalsIgnoreCase(abo) && MedwanQuery.getInstance().getConfigString("cntsBloodProducsNeedingABOCompatibility", "*cgr*st*").contains("*"+sProductCategory.toLowerCase()+"*")){
								%>
									<tr>
