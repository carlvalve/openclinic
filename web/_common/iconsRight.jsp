...
<%
    ...
    if(activePatient!=null && "1".equalsIgnoreCase((String)activePatient.adminextends.get("vip")) && !activeUser.getAccessRight("vipaccess.select")){
        ...
        if(sTmpPersonid.length() > 0){
            ...
            if(!bFingerPrint){
                %> <img class="link" onclick="enrollFingerPrint();" border="0" height='16px' src="<c:url value='/_img/icons/mobile/newfingerprint.png'/>" title="<%=getTranNoLink("web","enrollFingerPrint",sWebLanguage)%>"/><%
            }
            if(!bBarcode){
                %> <img class="link" onclick="printPatientCard();"  border='0' height='16px' src="<c:url value='/_img/icons/mobile/badge.png'/>" title="<%=getTranNoLink("web","printPatientCard",sWebLanguage)%>"/><%
            }
            if(!bPicture){
                %> <img class="link" onclick="storePicture();"  border='0' height='16px' src="<c:url value='/_img/icons/mobile/camera.png'/>" title="<%=getTranNoLink("web","loadPicture",sWebLanguage)%>"/><%
            }
        }
        ...
        if(activePatient!=null && activePatient.personid!=null && activePatient.personid.length()>0 && activeUser.getAccessRight("pharmacy.fastdispensing.select")){
            %><img class="link" onclick="showdrugsoutbarcode();"  height='16px' border='0' src="<c:url value='/_img/icons/icon_pharma.png'/>" title="<%=getTranNoLink("web","drugsoutbarcode",sWebLanguage)%>"/><%
        }
        %>
        ...
        <img class="link" onclick="initBarcode2();" border='0' height='16px' src="<c:url value='/_img/icons/mobile/qr.png'/>" title="<%=getTranNoLink("web","barcode",sWebLanguage)%>"/>
        <%
			if(activePatient!=null && activePatient.personid!=null && activePatient.personid.length()>0){
				if(MedwanQuery.getInstance().getConfigInt("enableImageHub",0)==1 && activePatient!=null && activePatient.comment5.length()>0){
                    %><img class="link" onclick="openImageHub()" border='0' height='16px' src="<c:url value='/_img/icons/icon_imagehub.png'/>" title="<%=getTranNoLink("web","imagehub",sWebLanguage)%>"/><%
				}
				else{
                    %><img class="link" onclick="showPACS()" border='0' height='16px' src="<c:url value='/_img/icons/mobile/xray.png'/>" title="<%=getTranNoLink("web","imagestore",sWebLanguage)%>"/><%
				}
		        if(activePatient!=null && activeUser.getAccessRight("labos.patientlaboresults.select") && activePatient.hasLabRequests()){
		            %><input type='button' accesskey=':' onclick="clickMenuItem('javascript:searchLab();');" style='display: none'/><img class="link" onclick="clickMenuItem('javascript:searchLab();');" title="<%=getTranNoLink("Web","labresults",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/results.png'/>"/><%
		        }
		        %><img class="link" onclick="clickMenuItem('<c:url value="/html5/login.jsp?page=getPatient.jsp&searchpersonid="/><%=activePatient.personid %>');" border='0' height='16px' src="<c:url value='/_img/icons/mobile/android.png'/>" title="<%=getTranNoLink("web","mobile.interface",sWebLanguage)%>"/>&nbsp;<%
			}
			else{
                %><img class="link" onclick="clickMenuItem('<c:url value="/html5/findPatient.jsp"/>');" border='0' height='16px' src="<c:url value='/_img/icons/mobile/android.png'/>" title="<%=getTranNoLink("web","mobile.interface",sWebLanguage)%>"/>&nbsp;<%
			}
        
        if(bMenu && activeUser.getAccessRight("labos.fastresults.select")){
            %><img class="link" onclick="clickMenuItem('javascript:showLabResultsEdit();');" title="<%=getTranNoLink("Web","fastresultsedit",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/lab.png'/>"/><%
        }
        if(bMenu && activeUser.getAccessRight("patient.administration.select")){
            %><img class="link" onclick="clickMenuItem('javascript:showAdminPopup();');" style="padding:2px" title="<%=getTranNoLink("Web","Administration",sWebLanguage)%>" height='16^px' src="<c:url value='/_img/icons/mobile/administration.png'/>"/><%
        }
    
        %><img class="link" onclick="clickMenuItem('javascript:searchMyHospitalized();');" title="<%=getTranNoLink("Web","my_hospitalized_patients",sWebLanguage)%>" title="<%=getTranNoLink("Web","my_hospitalized_patients",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/bed.png'/>"/><%
  
        if(activeUser.getAccessRight("manage.meals.select")){
            %>&nbsp;<img class="link" onclick="clickMenuItem('<c:url value="/main.do" />?Page=meals/index.jsp&ts='+new Date().getTime());" title="<%=getTranNoLink("web","meals",sWebLanguage)%>" title="<%=getTranNoLink("web","meals",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/meal.png'/>"/><%
        }
        
        %>
            <img class="link" onclick="clickMenuItem('javascript:searchMyVisits();');" title="<%=getTranNoLink("Web","my_on_visit_patients",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/doctor.png'/>"/>
            <img class="link" onclick="doPrint();" title="<%=getTranNoLink("Web","Print",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/print.png'/>" />
            <img class="link" id="ddIconAgenda" onclick="clickMenuItem('<c:url value='/main.do'/>?Page=planning/findPlanning.jsp&ts='+new Date().getTime());" title="<%=getTranNoLink("Web","Planning",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/calendar.png'/>"/>
        <%
        
        String sHelp = MedwanQuery.getInstance().getConfigString("HelpFile");
        if(sHelp.length() > 0){
            %><img id="ddIconHelp" height='16px' src="<c:url value='/_img/icons/mobile/question.png'/>" height="18" border="0" title="<%=getTranNoLink("Web","help",sWebLanguage)%>" onclick="showmanual();" onmouseover='this.style.cursor="hand"' onmouseout='this.style.cursor="default"'><%
        }
        
        %><img class="link" id="ddIconFingerprint" onclick="readFingerprint();" title="<%=getTranNoLink("Web","Read_fingerprint",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/fingerprint.png'/>"/><%
    }
%>

<img class="link" id="ddIconLogoff" onclick="clickMenuItem('javascript:confirmLogout();');" title="<%=getTranNoLink("Web","LogOff",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/logout.png'/>"/>
&nbsp;   