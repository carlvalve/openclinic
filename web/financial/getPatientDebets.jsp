                    ...
                    if(debet!=null){
                        ...
                        if(false){
	                        sReturn.append( "<tr class='list"+sClass+"'>"
		                            +"<td><input type='hidden' id='cbDebetInsurar"+debet.getUid()+"' value='"+new DecimalFormat(MedwanQuery.getInstance().getConfigString("priceCalculationFormat","#.00")).format(insurarAmount)+"'/><input type='hidden' name='cbDebet"+debet.getUid()+"="+new DecimalFormat(MedwanQuery.getInstance().getConfigString("priceCalculationFormat","#.00")).format(patientAmount)+"' "+sChecked+" "+(sChecked.length()>0?" value='1'":"")+"/>"+(sChecked.length()>0?"<img src='"+sCONTEXTPATH+"/_img/themes/default/checked.png'/>":"<img src='"+sCONTEXTPATH+"/_img/unchecked.png'/>")+"</td>"
