...
<form name='transactionForm' id='transactionForm' method='post'>
		...
		<tr>
			<td>
				<table width='100%' cellspacing='0'>
					...
					<%
						String sFullName="";
						AdminPerson person = AdminPerson.getAdminPerson(AdminPerson.getPersonIdByNatReg(fua.getAtencion().getValueString(73)));
				...
				<table width='100%' cellspacing='0'>
					<%
						for(int n=0;n<fua.getProcedimientos().size();n++){
							...
							System.out.println("CPT="+procedimientos.getValueString(2)+" ("+procedimientos.getValueString(11)+")");
	...
	%>
		...
		<script>
			...
			function readFingerprintFUA(){
			    <%
			        if(checkString(MedwanQuery.getInstance().getConfigString("referringServer")).length()==0){
			            %>openPopup("_common/readFingerPrintFUA.jsp&ts=<%=getTs()%>&referringServer=<%="http://"+request.getServerName()+"/"+sCONTEXTPATH%>",400,100);<%
			        }
			        else{
			            %>openPopup("_common/readFingerPrintFUA.jsp&ts=<%=getTs()%>&referringServer=<%=MedwanQuery.getInstance().getConfigString("referringServer")+sCONTEXTPATH%>",400,300);<%
			        }
			    %>
			}
			function loadList(){
