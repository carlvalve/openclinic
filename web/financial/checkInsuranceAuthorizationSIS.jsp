	...
	if(ScreenHelper.checkString(request.getParameter("forceautorization")).length()>0){
		...
		if(insurar.getAccreditationMechanism().equalsIgnoreCase("sis")){
			...
			if(checkString(insurance.getEstado()).equalsIgnoreCase("ACTIVO")){
				...
				accreditationresult="sis."+insurance.getIdError();
			}
		}
			...
			if(checkString(insurance.getEstado()).equalsIgnoreCase("ACTIVO")){
				//Check that this is the same patient
				if(!checkString(request.getParameter("ignorewarnings")).equalsIgnoreCase("true") 
						&& (!activePatient.lastname.toLowerCase().contains(insurance.getApePaterno().toLowerCase())
						|| !activePatient.lastname.toLowerCase().contains(insurance.getApeMaterno().toLowerCase())		
						|| !activePatient.firstname.toLowerCase().contains(insurance.getNombres().toLowerCase())		
						)){
					accreditationwarning=getTran(request,"web","patientname",sWebLanguage)+" <"+
							activePatient.lastname+", "+activePatient.firstname+"> "+getTran(request,"web","differsfrom",sWebLanguage)+" <"+
							insurance.getApePaterno()+" "+insurance.getApeMaterno()+", "+insurance.getNombres()+">";
				}
				else{
	...
	if(acreditacion!=null){
		...
		if(bValid){
			out.print(HTMLEntities.htmlentities("<td class='admin'><input type='hidden' id='authorized' value='1'/>"+ScreenHelper.getTran(request,"web","insurance.authorization",language)+"</td>"+
                       "<td class='admin2'>"+ScreenHelper.getTran(request,"web","authorized.by",language)+": <b>"+accreditationmechanism+"</b> "+ScreenHelper.getTran(request,"web","withauthorizationnumber",language)+" <b><a id='showdetailsanchor' href='javascript:showdetails(\""+acreditacion.getValueString(2)+"\")' title='"+getTranNoLink("web","details",sWebLanguage)+"'>"+acreditacion.getValueString(2)+"</b></td>"));
		}
	}
