...
<%
	...
	/// DEBUG /////////////////////////////////////////////////////////////////////////////////////
	if(Debug.enabled){
		Debug.println("\n*************** financial/extraInsuranceInvoiceEdit2.jsp **************");
    ...
    if(sFindInsurarInvoiceUID.length() > 0){
        insurarInvoice = ExtraInsurarInvoice2.getViaInvoiceUID(sFindInsurarInvoiceUID);
    ...
    <table class="menu" width="100%" cellpadding="0" cellspacing="1">
        <tr>
            ...
            <td class="admin2">
                ...
                <%-- BUTTONS --%>
                <input type="button" class="button" name="ButtonFind" value="<%=getTranNoLink("web","find",sWebLanguage)%>" onclick="doFind();void(0);">
                <input type="button" class="button" name="ButtonNew" value="<%=getTranNoLink("web","new",sWebLanguage)%>" onclick="doNew();searchInsurar();void(0);">
            </td>
        </tr>
    </table>
</form>
...
<script>
  ...
  function searchInsurarInvoice(){
    openPopup("/_common/search/searchExtraInsurarInvoice2.jsp&ts=<%=getTs()%>&doFunction=doFind()&ReturnFieldInvoiceNr=FindInsurarInvoiceUID&FindInvoiceInsurar=<%=sFindInsurarInvoiceUID%>");
  }
        ...
        <tr>
            ...
            <td class="admin2">
                <%
	                ...
	                <%
	                    String defaultmodel=MedwanQuery.getInstance().getConfigString("defaultExtraInvoiceModel2","default");
...
function doSave(){
  if((EditForm.EditDate.value.length > 0) && (EditForm.EditStatus.selectedIndex > -1 && EditForm.EditInsurarUID.value.length>0)){
    ...
    var url = '<c:url value="/financial/extraInsuranceInvoiceSave2.jsp"/>?ts='+new Date();
...
function doPrintPdf(invoiceUid){
  if(EditForm.PrintModel.value=='ramacsv'){
    var url = "<c:url value='/util/csvDocs.jsp'/>?invoiceuid="+invoiceUid+"&ts=<%=getTs()%>&docid=invoice.rama.extra2";
    window.open(url,"ExtraInsurarInvoicePdf<%=getTs()%>","height=600,width=900,toolbar=yes,status=no,scrollbars=yes,resizable=yes,menubar=yes");
 	...
 	var url = "<c:url value='/financial/createExtraInsurarInvoicePdf2.jsp'/>?InvoiceUid="+invoiceUid+"&ts=<%=getTs()%>&PrintLanguage="+EditForm.PrintLanguage.value+ "&PrintType="+EditForm.PrintType.value+"&PrintModel="+EditForm.PrintModel.value;
...
<%-- LOAD OPEN INSURAR INVOICES --%>
function loadOpenInsurarInvoices(){
  var url = '<c:url value="/financial/extraInsurarInvoiceGetOpenInsurarInvoices2.jsp"/>?ts='+new Date();
  new Ajax.Request(url,{
    method: "GET",
    parameters: "",
    onSuccess: function(resp){
...
function changeInsurar(){
  ...
  var url = '<c:url value="/financial/extraInsurarInvoiceGetPrestations2.jsp"/>?ts='+new Date();
  ...
  var url = '<c:url value="/financial/extraInsurarInvoiceGetCredits2.jsp"/>?ts='+new Date();
