...
<%
    ...
    if((activePatient!=null)&&(request.getParameter("SavePatientEditForm")==null)) {
      ...
      String sRelationship = "<select name='Comment5' select-one class='text'><option value=''/>"
              +ScreenHelper.writeSelect(request,"relationship",activePatient.comment5,sWebLanguage)+"</select>";
      ...
      out.print(normalRow("Web","gender","Gender","Admin",sWebLanguage)+sGender+"</select></td></tr>"
          ...
          +"<input type='hidden' name='Comment3' id='Comment3' value='"+activePatient.comment+"'/>"
          +normalRow("Web","headoffamily","Comment3Name","Admin",sWebLanguage)+"<input type='text' class='text' name='Comment3Name' id='Comment3Name' size='70' value='"+AdminPerson.getFullName(activePatient.comment3)+" "+activePatient.comment3+"'/><input class='button' type='button' value="+getTranNoLink("Web","select",sWebLanguage)+" onclick='showSearchPatientPopup(\"Comment3\",\"Comment3Name\");'></td></tr>"
          +normalRow("Web","relationship","Relationship","Admin",sWebLanguage)+sRelationship+"</td></tr>"
  	      +inputRow("Web","statute","Comment4","Admin",activePatient.comment,"T",true, false,sWebLanguage)
...
function showSearchPatientPopup(pid,personName){
    window.open("<c:url value="/popup.jsp"/>?Page=_common/search/searchPatient.jsp&ts=<%=getTs()%>&showPatientID=1&ReturnPersonID="+pid+"&ReturnName="+personName,"<%=getTran(null,"Web","Find",sWebLanguage)%>","height=1, width=1, toolbar=no, status=no, scrollbars=no, resizable=no, menubar=no");
  }
