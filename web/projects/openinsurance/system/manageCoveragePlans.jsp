        ...
        return "<tr id='rowCategory"+iTotal+"' class='"+(iTotal%2==0?"list":"list1")+"'>"+
                "<td>"+
                 "<a href='#' onclick='deleteCategory(rowCategory"+iTotal+");'>"+
                  "<img src='"+sCONTEXTPATH+"/_img/icon_delete.png' alt='"+getTranNoLink("Web","delete",sWebLanguage)+"' border='0'>"+
                 "</a>&nbsp;"+
                 "<a href='#' onclick='editCategory(rowCategory"+iTotal+");'>"+
                  "<img src='"+sCONTEXTPATH+"/_img/icon_edit.png' alt='"+getTranNoLink("Web","edit",sWebLanguage)+"' border='0'>"+
                 "</a>&nbsp;"+
                "</td>"+
               "<td>"+sCatName+"</td>"+
               "<td>"+sCatLabel+"</td>"+
...
<form id="transactionForm" name="transactionForm" method="post" <%=sOnKeyDown%> onclick="setSaveButton(event);" onkeyup="setSaveButton(event);">
    ...
    <%=writeTableHeader("Web.manage","ManageCoveragePlans",sWebLanguage," doBackToMenu();")%>
    ...
    <table width="100%" class="menu" cellspacing="0">
        <tr height="22">
            <td class="menu">
                ...
                <%-- BUTTONS --%>
                <img src="<c:url value="/_img/icon_delete.png"/>" class="link" alt="<%=getTranNoLink("Web","clear",sWebLanguage)%>" onclick="clearSearchFields();">&nbsp;
                        ...
                        <tbody>
                            <%
                                ...
                                for(int i=0; i<foundInsurars.size(); i++){
									...
									if(insurar.getContact().equalsIgnoreCase("plan.openinsurance")){
	                                    insurarCount++;
	                                    ...
	                                    %>
	                                        <tr class="list<%=sClass%>" onmouseover="this.style.cursor='hand';" onmouseout="this.style.cursor='default';">
	                                            <td>
	                                                <a href="#" onclick="deleteInsurar('<%=insurar.getUid()%>');"><img src='<c:url value="/_img/icon_delete.png"/>' border='0' alt="<%=sTranDelete%>"></a>
	                                            </td>
                ...
                <table width="100%" cellspacing="1" cellpadding="0" class="list">
                    ...
                    <tr>
                        ...
                        <td class="admin2">
                            ...
                            <input type="hidden" name="EditInsurarContact" value="plan.openinsurance"/>
                            <input type="hidden" name="EditInsurarLanguage" value="<%=MedwanQuery.getInstance().getConfigString("defaultLanguage","FR") %>"/>
                        </td>
                    </tr>
                    <%-- type --%>
                    <tr>
                    ...
                    <%-- SELECTED CATEGORIES --%>
                    <tr>
                        <td class="admin"><%=getTran(request,"web.manage","reimbursementcategories",sWebLanguage)%></td>
...
<script>
  ...
  function addCategoryToTable(catName,catLabel,catPatientShare,catuid){
    if(!allreadySelected(catName)){
      ...
      row.cells[0].innerHTML = "<a href='#' onclick='deleteCategory(rowCategory"+iIndexCategories+");'><img src='<%=sCONTEXTPATH%>/_img/icon_delete.png' alt='<%=getTranNoLink("Web","delete",sWebLanguage)%>' border='0'></a>&nbsp;"+
                               "<a href='#' onclick='editCategory(rowCategory"+iIndexCategories+");'><img src='<%=sCONTEXTPATH%>/_img/icon_edit.png' alt='<%=getTranNoLink("Web","edit",sWebLanguage)%>' border='0'></a>&nbsp;";
  ...
  function updateCategory(){
    if(isAtLeastOneCategoryFieldFilled()){
      ...
      row.cells[0].innerHTML = "<a href='#' onclick='deleteCategory("+editCategoryRowid.id+")'><img src='<%=sCONTEXTPATH%>/_img/icon_delete.png' alt='<%=getTran(null,"web","delete",sWebLanguage)%>' border='0'></a> "+
                               "<a href='#' onclick='editCategory("+editCategoryRowid.id+")'><img src='<%=sCONTEXTPATH%>/_img/icon_edit.png' alt='<%=getTran(null,"web","edit",sWebLanguage)%>' border='0'></a>";
  ...
  function doBack(){
    <%
        if(sAction.equals("edit") || sAction.equals("new")){
            %>
              if(checkSaveButton("<%=sCONTEXTPATH%>","<%=getTran(null,"Web.Occup","medwan.common.buttonquestion",sWebLanguage)%>")){
                  window.location.href = "<c:url value='/main.do'/>?Page=system/manageCoveragePlans.jsp";
              }
            <%
        }
        else{
