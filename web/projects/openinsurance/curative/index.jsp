...
<table width="100%" class="list">
    <tr><td colspan="10" class="titleadmin"><div style="float:left;vertical-align: middle"><%=getTran(request,"web","administrative.data",sWebLanguage)+" "+sVip%></div><%=getLastAccess("A."+activePatient.personid,sWebLanguage,request)%></td></tr>
<%
...
<script>
	var getAccessHistory = function(nb){
	    ...
	    Modalbox.show(url, {title: '<%=getTran(null,"web", "history", sWebLanguage)%>', width: 420,height:370},{evalScripts: true} );
	}
    var getAdminHistory = function(nb){
        var url = "<c:url value='/curative/ajax/getHistoryAdmin.jsp'/>?nb="+nb+"&ts="+new Date().getTime();
        Modalbox.show(url, {title: '<%=getTran(null,"web", "adminhistory", sWebLanguage)%>', width: 420,height:370},{evalScripts: true} );
    }
</script>