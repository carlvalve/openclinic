            ...
            <table width="100%" class='list' cellspacing="0" border="0">
                <tr class="admin">
                    <td width="300">
                        ...
                        <a href="<c:url value='/main.do'/>?Page=adt/historyEncounter.jsp&ts=<%=getTs()%>"><img src="<c:url value='/_img/icon_history2.gif'/>" class="link" alt="<%=getTran(null,"web","historyencounters",sWebLanguage)%>" style="vertical-align:-4px;"></a>
						...
						<%if(MedwanQuery.getInstance().getConfigString("quickConsult1."+activeUser.userid,"").length()>0){ %>
	                        <a href="javascript:newFastEncounter('<%=MedwanQuery.getInstance().getConfigString("quickConsult1."+activeUser.userid)%>');"><img src="<c:url value='/_img/icon_new1.png'/>" class="link" title="<%=getTranNoLink("web",MedwanQuery.getInstance().getConfigString("quickConsult1."+activeUser.userid).split(";")[0],sWebLanguage)+" "+getTranNoLink("service",MedwanQuery.getInstance().getConfigString("quickConsult1."+activeUser.userid).split(";")[2],sWebLanguage)%>" style="vertical-align:-4px;"></a>
	                    <%} %>
						<%if(MedwanQuery.getInstance().getConfigString("quickConsult2."+activeUser.userid,"").length()>0){ %>
	                        <a href="javascript:newFastEncounter('<%=MedwanQuery.getInstance().getConfigString("quickConsult2."+activeUser.userid)%>');"><img src="<c:url value='/_img/icon_new2.png'/>" class="link" title="<%=getTranNoLink("web",MedwanQuery.getInstance().getConfigString("quickConsult2."+activeUser.userid).split(";")[0],sWebLanguage)+" "+getTranNoLink("service",MedwanQuery.getInstance().getConfigString("quickConsult2."+activeUser.userid).split(";")[2],sWebLanguage)%>" style="vertical-align:-4px;"></a>
	                    <%} %>
        ...
        <td>
            <table width='100%' cellspacing='0' cellpadding="0" class='list'>
                ...
                <tr class='list' onmouseover="this.style.cursor='hand';"
                    ...
                    <td>
                        <img src="<c:url value='/_img/icon_edit.png'/>" class="link" alt="<%=getTran(null,"web","edit",sWebLanguage)%>" onclick="goEdit('<%=activeEncounter.getUid()%>');">
