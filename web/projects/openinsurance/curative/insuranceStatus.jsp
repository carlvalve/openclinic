...
<table class="list" width="100%" cellspacing="0">
    <tr class="admin">
        <td>
            ...
            <a href="<c:url value='/main.jsp'/>?Page=financial/insurance/editInsurance.jsp&ts=<%=getTs()%>"><img src="<c:url value='/_img/icons/icon_new.png'/>" class="link" alt="<%=getTranNoLink("web","newinsurance",sWebLanguage)%>" style="vertical-align:-4px;"></a>
        </td>
    </tr>
    <%
        ...
        <td style="padding:0;">
            <table width="100%" class="sortable" cellpadding="0" cellspacing="0" id="searchresultsInsurance" style="border:0;">
                <tr class="gray">
                    ...
                    <td><%=getTran(request,"web","validity",sWebLanguage)%></td>
                </tr>
    <%
    ...
    %>
                <tr class="list<%=sClass%>" onmouseover="this.style.cursor='hand';"
                    ...
                    <td onclick="doSelect('<%=currentInsurance.getUid()%>');"><%=ScreenHelper.checkString(getTran(request,"insurance.types",currentInsurance.getType(),sWebLanguage))%></td>
