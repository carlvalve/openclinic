...
<%
    ...
    if (sEditDebetUID.length() > 0) {
        ...
        Encounter encounter = Encounter.getCoverageEncounter(activePatient.personid,activeUser.userid);
        if (encounter != null) {
            sEditEncounterName = encounter.getEncounterDisplayName(sWebLanguage);
...
<script>
  ...
  function doSave(){
    ...
    function searchPrestation(){
        ...
        openPopup("/_common/search/searchContribution.jsp&ts=<%=getTs()%>&ReturnFieldUid=tmpPrestationUID&ReturnFieldDescr=tmpPrestationName&doFunction=changeTmpPrestation()");
    }
