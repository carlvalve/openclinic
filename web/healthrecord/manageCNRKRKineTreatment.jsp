...
<table class="list" cellspacing="1" cellpadding="0" width="100%">
    ...
    <tr class="admin" style="padding:0px;">
        ...
        <td class="admin2" colspan='4'>
            <textarea onKeyup="resizeTextarea(this,10);limitChars(this,5000);" <%=setRightClick("ITEM_TYPE_CNRKR_KINE_TREATMENTCOMMENTS")%> class="text" cols="80" rows="2" name="currentTransactionVO.items.<ItemVO[hashCode=<mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNRKR_KINE_TREATMENTCOMMENTS" property="itemId"/>]>.value"><mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNRKR_KINE_TREATMENTCOMMENTS" property="value"/></textarea>
        </td>
    </tr>
	<tr class='admin'>
		<td width='50px'><img src='<%=sCONTEXTPATH %>/_img/icons/icon_new.gif' onclick='addTreatmentLine()'/></td>
		<td width='15%'><%=getTran(request,"web","date",sWebLanguage) %></td>
		<td width='15%'><%=getTran(request,"web","act",sWebLanguage) %></td>
		<td width='15%'><%=getTran(request,"web","act",sWebLanguage) %></td>
		<td width='15%'><%=getTran(request,"web","act",sWebLanguage) %></td>
...
<script>
	function addTreatment(date,act1,act2,act3,comment){
		if(document.getElementById('treatments').value.length>0){
			document.getElementById('treatments').value+="£";
		}
		document.getElementById('treatments').value += date+";"+act1+";"+act2+";"+act3+";"+comment;
		writeTreatments();
	}
	...
	function editTreatment(row,date,act1,act2,act3,comment){
		var newTreatments="";
		var treatments = document.getElementById('treatments').value.split("£");
		for(n=0;n<treatments.length;n++){
			if(n>0){
			...
			if(n==row){
				newTreatments+= date+";"+act1+";"+act2+";"+act3+";"+comment;
			}
			else{
	...
	function writeTreatments(){
	    ...
	    var today = new Date();
	    var url= '<c:url value="/healthrecord/getKineTreatments.jsp"/>?treatments='+document.getElementById('treatments').value+'&ts='+today;
	...
	function addTreatmentLine(){
	  openPopup("/healthrecord/editKineTreatment.jsp&new=true&language=<%=sWebLanguage%>");
	}
	function editTreatmentLine(row){
		...
		for(n=0;n<treatments.length;n++){
			if(n==row){
				...
				var act1 = treatments[n].split(";")[1];
				var act2 = treatments[n].split(";")[2];
				var act3 = treatments[n].split(";")[3];
				var comment = treatments[n].split(";")[4];
				openPopup("/healthrecord/editKineTreatment.jsp&new=false&language=<%=sWebLanguage%>&row="+row+"&date="+date+"&act1="+act1+"&act2="+act2+"&act3="+act3+"&comment="+comment);
				break;
			}
		}
	}
