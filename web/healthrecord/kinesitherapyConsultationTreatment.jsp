...
<%!
    ...
    private StringBuffer addSeance(int iTotal, String sDate, String sHour, 
        ...
        if(sTreatment1.length() > 0){
            sTreatment1 = getTran(null,"physiotherapy.act",sTreatment1,sWebLanguage);
        }

        if(sTreatment2.length() > 0){
            sTreatment2 = getTran(null,"physiotherapy.act",sTreatment2,sWebLanguage);
        }

        if(sTreatment3.length() > 0){
            sTreatment3 = getTran(null,"physiotherapy.act",sTreatment3,sWebLanguage);
        }
        ...
        sTmp.append("<tr id='rowSeance"+iTotal+"' class='list"+sClass+"' onMouseOver=\"this.className='list_select'\" onMouseout=\"this.className='list"+sClass+"'\">")
             .append("<td>")
              .append("<a href='javascript:deleteSeance(rowSeance"+iTotal+")'><img src='"+sCONTEXTPATH+"/_img/icons/icon_delete.png' alt='"+getTranNoLink("Web.Occup","medwan.common.delete",sWebLanguage)+"' border='0'></a> ")
              .append("<a href='javascript:editSeance(rowSeance"+iTotal+")'><img src='"+sCONTEXTPATH+"/_img/icons/icon_edit.png' alt='"+getTranNoLink("Web.Occup","medwan.common.edit",sWebLanguage)+"' border='0'></a>")
             .append("</td>")
             .append("<td>&nbsp;"+sDate+"</td>")
             .append("<td>&nbsp;"+sHour+"</td>")
             .append("<td>&nbsp;"+sTreatment1+"</td>")
             .append("<td>&nbsp;"+sTreatment2+"</td>")
             .append("<td>&nbsp;"+sTreatment3+"</td>")
             .append("<td></td>")
            .append("</tr>");
...
<form id="transactionForm" name="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" cellspacing="1" cellpadding="0" width="100%">
        ...
        <%-- CONTEXT --%>
        <tr>
            <td class="admin"><%=getTran(request,"openclinic.chuk","context",sWebLanguage)%></td>
            <td class="admin2">
        ...
        <%-- PHYSIO THERAPY --%>
        <tr>
            <td class="admin"><%=getTran(request,"openclinic.chuk","physio.therapy",sWebLanguage)%></td>
            <td class="admin2">
        ...
        <%-- ICPC / ICD10 --%>
        <tr>
            <td class="admin"><a href="javascript:openPopup('healthrecord/findICPC.jsp&ts=<%=getTs()%>&patientuid=<%=activePatient.personid %>');void(0);"><%=getTran(request,"openclinic.chuk","diagnostic",sWebLanguage)%> <%=getTran(request,"Web.Occup","ICPC-2",sWebLanguage)%>/<%=getTran(request,"Web.Occup","ICD-10",sWebLanguage)%></a></td>
        ...
        <%-- START DATE --%>
        <tr>
            <td class="admin"><%=getTran(request,"openclinic.chuk","startdate",sWebLanguage)%></td>
            <td class="admin2">
        ...
        <%-- SEANCES SCEME --%>
        <tr>
            <td class="admin"><%=getTran(request,"openclinic.chuk","date.seances",sWebLanguage)%></td>
            <td class="admin2">                
                <table id="searchresults" width="100%" class="sortable" cellspacing="0" headerrowcount="2">
                    <%-- HEADER --%>
                    <tr class="admin">
                        <td width="38">&nbsp;</td>
                        <td width="140"><%=getTran(request,"Web.occup","medwan.common.date",sWebLanguage)%></td>
                        <td width="55"><%=getTran(request,"Web.occup","medwan.common.hour",sWebLanguage)%></td>
                        <td><%=getTran(request,"openclinic.chuk","treatment",sWebLanguage)%></td>
                        <td><%=getTran(request,"openclinic.chuk","treatment",sWebLanguage)%></td>
                        <td><%=getTran(request,"openclinic.chuk","treatment",sWebLanguage)%></td>
                        <td></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="admin">
                            <select class="text" name="seancetreatment1">
                                <option/>
                                <%=ScreenHelper.writeSelect(request,"physiotherapy.act","",sWebLanguage)%>
                            </select>
                        </td>
                        <td class="admin">
        ...
        <%-- END DATE --%>
        <tr>
            <td class="admin"><%=getTran(request,"openclinic.chuk","enddate",sWebLanguage)%></td>
            <td class="admin2">
        ...
        <%-- REFERENCE --%>
        <tr>
            <td class="admin"><%=getTran(request,"openclinic.chuk","reference",sWebLanguage)%></td>
            <td class="admin2">
...
function addSeance(){
  if(isAtLeastOneSeanceFieldFilled()){
    ...
    var tr = searchresults.insertRow(searchresults.rows.length);
    tr.id = "rowSeance"+iSeancesIndex;
    tr.onmouseover = "this.className = 'list_select'";
    ...
    td = tr.insertCell(3);
    if(transactionForm.seancetreatment1.value.length > 0){
    ...
    td = tr.insertCell(4);
    if(transactionForm.seancetreatment2.value.length > 0){
    ...
    td = tr.insertCell(5);
    if(transactionForm.seancetreatment3.value.length > 0){
...
function updateSeance(){
  if(isAtLeastOneSeanceFieldFilled()){
    ...
    setCellStyle(row,editSeancesRowid.rowIndex);
    clearSeanceFields();
    transactionForm.ButtonUpdateSeance.disabled = true;
  }
}
...
<%-- SET CELL STYLE --%>
function setCellStyle(row,idx){
  row.className = "list"+(idx%2==0?"":"1");
  row.onmouseover = function(){this.className = 'list_select'};
  row.onmouseout = function(){this.className = 'list'+(idx%2==0?"":"1")};
}
