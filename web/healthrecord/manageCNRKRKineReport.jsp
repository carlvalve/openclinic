...
<table class="list" cellspacing="1" cellpadding="0" width="100%">
    <tr class="admin" style="padding:0px;">
        <td colspan="5"><%=getTran(request,"web","evaluations",sWebLanguage)%></td>
    </tr>
	<tr class='admin' style="padding:0px;">
		<td width='30px'><img src='<%=sCONTEXTPATH %>/_img/icons/icon_new.gif' onclick='addEvaluationLine()'/></td>
		<td width='20%'><%=getTran(request,"web","date",sWebLanguage) %></td>
    ...
    <tr class="admin" style="padding:0px;">
        <td colspan="5"><%=getTran(request,"web","finalreport",sWebLanguage)%></td>
    </tr>
    <tr>
...
<script>
	...
	function editEvaluation(row,date,objectives,functional,comment){
		var newEvaluations="";
		var evaluations = document.getElementById('evaluations').value.split("£");
		for(n=0;n<evaluations.length;n++){
			if(n>0){
			...
			if(n==row){
				newEvaluations+= date+";"+objectives+";"+functional+";"+comment;
			}
			else{
	...
	function writeEvaluations(){
	    ...
	    var today = new Date();
	    var url= '<c:url value="/healthrecord/getKineEvaluations.jsp"/>?evaluations='+document.getElementById('evaluations').value+'&ts='+today;
	...
	function addEvaluationLine(){
	  openPopup("/healthrecord/editKineEvaluation.jsp&new=true&language=<%=sWebLanguage%>");
	}
	function editEvaluationLine(row){
		...
		for(n=0;n<evaluations.length;n++){
			if(n==row){
				...
				openPopup("/healthrecord/editKineEvaluation.jsp&new=false&language=<%=sWebLanguage%>&row="+row+"&date="+date+"&objectives="+objectives+"&functional="+functional+"&comment="+comment);
				break;
			}
		}
	}
