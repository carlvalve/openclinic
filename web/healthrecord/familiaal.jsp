...
<%!
    ...
    private String addKinderen(int iTotal, String sTmpKinderenGeboortejaar, String sTmpKinderenGeslacht, String sTmpKinderenFirstname, String sWebLanguage) {
        ...
        sOut.append("<tr id='rowKinderen" + iTotal + "'>")
                .append("<td width='36'>")
                .append("<a href='javascript:deleteKinderen(rowKinderen" + iTotal + ")'><img src='" + sCONTEXTPATH + "/_img/icons/icon_delete.gif' alt='" + getTran(null,"Web.Occup", "medwan.common.delete", sWebLanguage) + "' border='0'></a> ")
                .append("<a href='javascript:editKinderen(rowKinderen" + iTotal + ")'><img src='" + sCONTEXTPATH + "/_img/icons/icon_edit.gif' alt='" + getTran(null,"Web.Occup", "medwan.common.edit", sWebLanguage) + "' border='0'></a>")
                .append("</td>");
...
<%
    ...
    if (transaction != null){
        ...
        if (tran!=null){
            sKinderen         = getItemType(tran.getItems(),"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CE_KINDEREN");
            sFami             = getItemType(tran.getItems(),"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CE_FAMILIALE_ANTECEDENTEN");
            sStatus           = getItemType(tran.getItems(),"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CE_STATUS");
            sStatusComment    = getItemType(tran.getItems(),"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CE_STATUS_COMMENT");
            sFamiliaalComment = getItemType(tran.getItems(),"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CE_FAMILIAAL_COMMENT");
        }

        // herneem data van vorig klinisch onderzoek of RPR
        if (tran.getTransactionId().intValue()<0){
            if (sKinderen.length() == 0){
                sKinderen+= getLastItemValue(request,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CE_KINDEREN");
            }

            if (sFami.length() == 0){
                sFami+= getLastItemValue(request,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CE_FAMILIALE_ANTECEDENTEN");
            }

            if (sStatus.length() == 0){
                sStatus+= getLastItemValue(request,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CE_STATUS");
            }

            sStatusComment+= getLastItemValue(request,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CE_STATUS_COMMENT");

            sFamiliaalComment+= getLastItemValue(request,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CE_FAMILIAAL_COMMENT");
            for(int i=1; i<8; i++){
                sFamiliaalComment+= getLastItemValue(request,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CE_FAMILIAAL_COMMENT"+i);
            }
                    ...
                    <td colspan="2">
                        <table width="100%" cellspacing="1" id="tblKinderen">
                            ...
                            <tr>
                                ...
                                <td class="admin2">
                                    <input type="button" class="button" name="ButtonAddKinderen" value="<%=getTranNoLink("Web","add",sWebLanguage)%>" onclick="addKinderen()">
                                    <input type="button" class="button" name="ButtonUpdateKinderen" value="<%=getTranNoLink("Web","edit",sWebLanguage)%>" onclick="updateKinderen()">
                                </td>
                            </tr>
...
function addKinderen(){
  if(isAtLeastOneKinderenFieldFilled()){
    ...
    if(birthDate.getTime() > today.getTime()){
      var popupUrl = "<%=sCONTEXTPATH%>/_common/search/template.jsp?Page=okPopup.jsp&ts=<%=getTs()%>&labelType=Web.Occup&labelID=futureDateNotAllowed";
      ...
      if(window.showModalDialog){
          ...
          window.confirm("<%=getTran(null,"Web.occup","futureDateNotAllowed",sWebLanguage)%>");
      }
      ...
      td.innerHTML = "<a href='javascript:deleteKinderen(rowKinderen"+iIndex+")'><img src='<%=sCONTEXTPATH%>/_img/icons/icon_delete.gif' alt='<%=getTranNoLink("Web.Occup","medwan.common.delete",sWebLanguage)%>' border='0'></a> "
                                  +"<a href='javascript:editKinderen(rowKinderen"+iIndex+")'><img src='<%=sCONTEXTPATH%>/_img/icons/icon_edit.gif' alt='<%=getTranNoLink("Web.Occup","medwan.common.edit",sWebLanguage)%>' border='0'></a>";
      tr.appendChild(td);
...
function updateKinderen(){
  if(isAtLeastOneKinderenFieldFilled()){
    ...
    row.cells[0].innerHTML = "<a href='javascript:deleteKinderen("+editKinderenRowid.id+")'><img src='<%=sCONTEXTPATH%>/_img/icons/icon_delete.gif' alt='<%=getTranNoLink("Web.Occup","medwan.common.delete",sWebLanguage)%>' border='0'></a> "
                            +"<a href='javascript:editKinderen("+editKinderenRowid.id+")'><img src='<%=sCONTEXTPATH%>/_img/icons/icon_edit.gif' alt='<%=getTranNoLink("Web.Occup","medwan.common.edit",sWebLanguage)%>' border='0'></a>";
