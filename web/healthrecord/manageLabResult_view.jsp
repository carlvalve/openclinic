...
<%
    ...
    while(iterator.hasNext()){
        ...
        while(enumeration.hasMoreElements()){
            ...
            if(MedwanQuery.getInstance().getConfigString("hiddenLabAnalyses","").contains("*"+requestedLabAnalysis.getAnalysisCode()+"*")){
            	continue;
            }
...
<script>
  ...
  function printOrder(serverid,transactionid){
	    window.open("<c:url value='/labos/createLabResultsPdf.jsp'/>?ts=<%=getTs()%>&order=1&print."+serverid+"."+transactionid+"=1","Popup"+new Date().getTime(),"toolbar=no,status=yes,scrollbars=yes,resizable=yes,width=800,height=600,menubar=no");
	  }
