...
<%=checkPermission("occup.death.audit","select",activeUser)%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
	        	...
	        	<table width='100%'>
		        	<tr>
			            ...
			            <td class='admin2'>
	                        ...
	                        <script>writeMyDate("deathdate", "<c:url value="/_img/icons/icon_agenda.gif"/>", "<%=getTran(null,"Web","PutToday",sWebLanguage)%>");</script>
			            </td>
		        	...
		        	<tr>
			            ...
			            <td class="admin2">
			                ...
			                <mxs:propertyAccessorI18N name="transaction.items" scope="page"
			                                          compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_MSAS_DECEASED_AUDIT;value=medwan.common.true"
			                                          property="value" outputString="checked"/>><label><%=getTran(request,"web","yes",sWebLanguage) %></label>
			                <input type="radio" onDblClick="uncheckRadio(this);" name="currentTransactionVO.items.<ItemVO[hashCode=<mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_MSAS_DECEASED_AUDIT" property="itemId"/>]>.value" value="medwan.common.false"
			                <mxs:propertyAccessorI18N name="transaction.items" scope="page"
	...
	<%=ScreenHelper.alignButtonsStart()%>
	    <%=getButtonsHtml(request,activeUser,activePatient,"occup.death.audit",sWebLanguage)%>
