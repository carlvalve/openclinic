...
%>
        ...
        <table border="0" width="100%">
            <tr>
                ...
                <td width="100" align="left"><script>document.write(printDate+"/"+printMonth+"/"+today.getFullYear());</script></td>
                <td width="220" align="left"><%=sFVCBeginDate%></td>
                <td width="*"   align="left"><script>document.write(printDate+"/"+printMonth+"/"+today.getFullYear());</script></td>
            </tr>
        </table>
...
<script>
  ...
  function loadSpirobank(){
    var url = "<c:url value='/popup.jsp'/>?Page=/healthrecord/loadSpirobank.jsp&load=yes";
  ...
  function getFEV1Normal(iTmpDate){
    var iTmpAge = iTmpDate.substring(iTmpDate.lastIndexOf("/")+1)-yearOfBirth;
    var iTmp = 0.0;
  ...
  <%-- DRAW FEV1 GRAPH --%>
  var aFEV1Value = new Array(<%=sFEV1Value%>);
  var aFEV1Date = new Array(<%=sFEV1Date%>);
  ...
  if(aFEV1Date.length>0){
    ...
    dFEV1Today = new Dot(DFEV1.ScreenX(Date.UTC(today.getFullYear(),today.getMonth()+1,today.getDate(),0,0,0,0)),DFEV1.ScreenY(document.getElementsByName('MyFEV1')[0].value),9,5,"32CD32",document.getElementsByName('MyFEV1')[0].value);
  }

  <%-- DRAW FVC GRAPH --%>
  var aFVCValue = new Array(<%=sFVCValue%>);
  var aFVCDate = new Array(<%=sFVCDate%>);
  ...
  if(aFVCValue.length>0){
    ...
    for (i=0; i<aFVCValue.length; i++){
      var iX = Date.UTC(aFVCDate[i].substr(aFVCDate[i].lastIndexOf("/")+1),aFVCDate[i].substr(aFVCDate[i].indexOf("/")+1,2),aFVCDate[i].substr(0,aFVCDate[i].indexOf("/")),0,0,0);
      ...
      if(i<aFVCValue.length-1){
        var iXNext = Date.UTC(aFVCDate[i+1].substr(aFVCDate[i+1].lastIndexOf("/")+1),aFVCDate[i+1].substr(aFVCDate[i+1].indexOf("/")+1,2),aFVCDate[i+1].substr(0,aFVCDate[i+1].indexOf("/")),0,0,0);
    ...
    var dFVCToday = new Dot(DFVC.ScreenX(Date.UTC(today.getFullYear(),today.getMonth()+1,today.getDate(),0,0,0,0)),DFVC.ScreenY(document.getElementsByName('MyFVC')[0].value),9,5,"32CD32",document.getElementsByName('MyFVC')[0].value);
  }
  ...
  function calculateToday(){
    ...
    if(aFEV1Value.length>0){
      ...
      dFEV1Today.MoveTo(DFEV1.ScreenX(Date.UTC(today.getFullYear(),today.getMonth()+1,today.getDate(),0,0,0)),DFEV1.ScreenY(iFEVValue));
    }
    ...
    if(aFVCValue.length>0){
      ...
      dFVCToday.MoveTo(DFVC.ScreenX(Date.UTC(today.getFullYear(),today.getMonth()+1,today.getDate(),0,0,0)),DFVC.ScreenY(iFVCValue));
    }
  }
