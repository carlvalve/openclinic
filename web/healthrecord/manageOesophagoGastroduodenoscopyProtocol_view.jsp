...
<form name="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1">
        <tr class="admin">
            <td colspan="4"><%=getTran(request,"openclinic.chuk","cardio",sWebLanguage)%></td>
        </tr>
        <tr>
...
<%-- BUTTONS --%>
    <%
      if (activeUser.getAccessRight("occup.protocol.oesophagogastroduodenoscopy.add") || activeUser.getAccessRight("occup.protocol.oesophagogastroduodenoscopy.edit")){
    %>
