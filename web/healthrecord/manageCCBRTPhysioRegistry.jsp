...
<%=checkPermission("occup.ccbrt.physioregistry","select",activeUser)%>

<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    <bean:define id="transaction" name="be.mxs.webapp.wl.session.SessionContainerFactory.WO_SESSION_CONTAINER" property="currentTransactionVO"/>
	<%=checkPrestationToday(activePatient.personid,false,activeUser,(TransactionVO)transaction)%>
   	<%
   		System.out.println("iiiiiiiiiiiiiii="+((TransactionVO)transaction).getServerId());
   	%>
	        	...
	        	<table width='100%'>
			        ...
			        <tr>
			            ...
			            <td class="admin2" colspan="6">
							...
							<img src='<c:url value="/_img/icons/icon_search.gif"/>' class='link' alt='<%=getTranNoLink("Web","select",sWebLanguage)%>' onclick='searchService("referral","referralname");'>				            
							<img src='<c:url value="/_img/icons/icon_delete.gif"/>' class='link' alt='<%=getTranNoLink("Web","clear",sWebLanguage)%>' onclick='document.getElementById("referral").value="";document.getElementById("referralname").value="";'>				            
						</td>
			        </tr>
		        	<tr>
