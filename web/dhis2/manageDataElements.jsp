...
<%
	...
	String sModifiers = ScreenHelper.makeArrayFromParameters(request,"modifiers",";");
	...
	if(request.getParameter("saveButton")!=null){
		...
		PreparedStatement ps = conn.prepareStatement("delete from OC_DHIS2DataElements where uid=?");
		ps.setString(1,sUid);
		ps.execute();
		ps=conn.prepareStatement("insert into OC_DHIS2DataElements(type,uid,label,minimum,maximum,equals,comment,modifiers) values(?,?,?,?,?,?,?,?)");
...
<form name='transactionForm' method='post'>
	...
	<table width='100%'>
		<tr class='admin'><td><%=getTran(request,"dhis2","managedataelements",sWebLanguage) %></td></tr>
		<tr>
			<td>
				<table width='100%'>
					<tr>
						...
						<td class='admin2'>
							<select class='text' name='type' id='type' onchange='clearFields();transactionForm.submit();'>
								<option/>
								<%=ScreenHelper.writeSelect(request, "dhis2.dataelementtype", sType, sWebLanguage) %>
							</select>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td class='admin'><%=getTran(request,"web","modifiers",sWebLanguage) %> *</td>
						<td class='admin2'>
							<%=ScreenHelper.writeCheckBoxes(request, "dhis2.dataelementmodifiers", "modifiers", sModifiers, sWebLanguage, true) %>
						</td>
					</tr>
					<tr>
...
<%
	Connection conn = MedwanQuery.getInstance().getOpenclinicConnection();
	PreparedStatement ps = conn.prepareStatement("select * from OC_DHIS2DataElements where type=? order by label");
	ps.setString(1,sType);
	...
	while(rs.next()){
		...
		out.println("<img src='"+sCONTEXTPATH+"/_img/icons/icon_delete.gif' onclick='deleteItem(\""+uid+"\")'/>");
		out.println("<img src='"+sCONTEXTPATH+"/_img/icons/icon_edit.gif' onclick='editItem(\""+type+"\",\""+uid+"\",\""+label+"\",\""+min+"\",\""+max+"\",\""+val+"\",\""+comment+"\",\""+modifiers+"\")'/>");
		out.println("</td>");
...
<script>
	...
	function editItem(type,uid,label,min,max,val,comment,modifiers){
		...
		for(n=0;n<mods.length;n++){
			document.getElementById('modifiers.'+mods[n]).checked=true;
		}
	}
	...
	function clearFields(){
		...
		for(n=0;n<els.length;n++){
			if(els[n] && els[n].name && els[n].name.indexOf("modifiers.")>-1){
				els[n].checked=false;
			}
		}
	}
