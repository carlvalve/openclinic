...
<%
    ...
    if(sAction.equalsIgnoreCase("save")){
    	...
    	nomenclature.setType("assetcomponent");
    	...
    	while(parnames.hasMoreElements()){
    		...
    		if(parname.startsWith("label.")){
    			...
    			label.type="admin.nomenclature.assetcomponent";
    	...
    	Nomenclature.delete("assetcomponent",checkString(request.getParameter("code")));
    	Label.delete("admin.nomenclature.assetcomponent", checkString(request.getParameter("code")));
    	sFindText="";
    }
%>
...
<form name="transactionForm" method="post">
	...
	<table width="100%">
		<tr class="admin">
			<td colspan="2"><%=getTran(request,"web","manageAssetComponentNomenclature",sWebLanguage) %></td>
		</tr>
		<tr>
			...
			<td class='admin2'>
                ...
                <img src="<c:url value="/_img/icons/icon_search.gif"/>" class="link" alt="<%=getTranNoLink("Web","select",sWebLanguage)%>" onclick="searchNomenclature('FindNomenclatureCode','FindText');">
                <img src="<c:url value="/_img/icons/icon_delete.gif"/>" class="link" alt="<%=getTranNoLink("Web","clear",sWebLanguage)%>" onclick="transactionForm.FindNomenclatureCode.value='';transactionForm.FindText.value='';">
	...
	<%
		...
		if(sAction.equalsIgnoreCase("find")){
			Vector nomenclatureIDs = Nomenclature.getNomenclatureIDsByText("assetcomponent", sWebLanguage, sFindText);
			boolean bInitialized=false;
			for(int n=0;n<nomenclatureIDs.size();n++){
				bInitialized=true;
				String id = (String)nomenclatureIDs.elementAt(n);
				out.println("<tr><td class='admin'><a href='javascript:edit(\""+id+"\")'>"+id+"</a></td><td class='admin2'>"+getTran(request,"admin.nomenclature.assetcomponent",id,sWebLanguage)+"</td></tr>");
			}
			if(!bInitialized){
			...
			nomenclature=Nomenclature.get("assetcomponent",checkString(request.getParameter("editcode")));
		}
		...
		%>
			...
			<tr>
				...
				<td class='admin2'>
	                ...
	                <img src="<c:url value="/_img/icons/icon_search.gif"/>" class="link" alt="<%=getTranNoLink("Web","select",sWebLanguage)%>" onclick="searchNomenclature('parentcode','FindParentText');">
	                <img src="<c:url value="/_img/icons/icon_delete.gif"/>" class="link" alt="<%=getTranNoLink("Web","clear",sWebLanguage)%>" onclick="transactionForm.parentcode.value='';transactionForm.FindParentText.value='';">
				</td>
			</tr>
			<%
			    ...
			    while(tokenizer.hasMoreTokens()){
			        ...
			        %>
			            <tr>
			                ...
			                <td class="admin2">
			                    <input type="text" class="text" name="label.<%=tmpLang%>" value="<%=getTranNoLink("admin.nomenclature.assetcomponent",nomenclature.getId(),tmpLang)%>" size="80">
			                </td>
			            </tr>
			        <%
			    }
...
<script>
    ...
    function searchNomenclature(CategoryUidField,CategoryNameField){
	    openPopup("/_common/search/searchNomenclature.jsp&ts=<%=getTs()%>&FindType=assetcomponent&VarCode="+CategoryUidField+"&VarText="+CategoryNameField);
	}
