<%@ page import="java.util.Hashtable,java.util.Enumeration,java.util.StringTokenizer" %>
...
<%
    final String sLabelType = "resultprofiles";
...
<%=writeTableHeader("Web.Occup","medwan.system-related-actions.manage-resultProfiles",sWebLanguage,"doBack();")%>
<table width="100%" class="menu" cellspacing="0" cellpadding="0">
  <%-- INPUT & BUTTONS --%>
  <tr>
    <td width="<%=sTDAdminWidth%>"> <%=getTran(request,"Web.manage","labprofiles.cols.code",sWebLanguage)%></td>
    <td>
...
<%
    ...
    if (sAction.equals("save") || sAction.equals("new")) {
        if (sEditProfileCode.length() > 0) {
            ...
            // execute query
            if (!recordExists) {
                ResultsProfile labProfile = new ResultsProfile();
    ...
    if (sAction.equals("delete")) {
        ...
        ResultsProfileAnalysis.deleteByProfileID(Integer.parseInt(sProfileID));
        // message
    ...
    if (sAction.equals("addLab")) {
        ...
        // compose query
        boolean connected = ResultsProfile.checkLabProfileLabAnalysisConnection(sLabCodeOther, sLabComment, sLabID, sProfileID, sLabCode);

        if (!connected) {
            ResultsProfileAnalysis labProfAnalysis = new ResultsProfileAnalysis();
            ...
            ResultsProfile.updateUpdateTime(Integer.parseInt(sProfileID));
    ...
    if (sAction.equals("remLab")) {
        ...
        ResultsProfileAnalysis.deleteByProfileIDLabID(Integer.parseInt(sProfileID), Integer.parseInt(sLabID));
        // set updatetime of labprofile to now
        ResultsProfile.updateUpdateTime(Integer.parseInt(sProfileID));
        // message
        if (sLabCodeOther.equals("1")) {
...
%>
        <table width="100%" cellspacing="0" cellpadding="0" class="sortable" id="searchresults">
          <tr class="admin">
            <td width='15%'>&nbsp;<%=getTran(request,"Web.manage","labprofiles.cols.code",sWebLanguage)%></td>
            <td width='30%'>&nbsp;<%=getTran(request,"Web","description",sWebLanguage)%></td>
            <td width='*%'>&nbsp;<%=getTran(request,"Web.manage","labprofiles.cols.comment",sWebLanguage)%></td>
          </tr>
        ...
        Vector vLabProfiles = ResultsProfile.getActiveLabProfilesByProfileCode(sFindProfileCode);
        ...
        while(iter.hasNext()){
            labProfile = (ResultsProfile)iter.next();
            iTotal++;
          ...
          <table border="0" width="100%">
            <tr height="30">
              ...
              <td align="right">
                ...
                <a  href="<c:url value="/main.do"/>?Page=system/manageLabAnalyses.jsp&ts=<%=getTs()%>" onMouseOver="window.status='';return true;"><%=getTran(request,"Web.Occup","medwan.system-related-actions.manage-labAnalysis",sWebLanguage)%></a>&nbsp;
              </td>
            </tr>
          </table>
        <%
    ...
    if(sAction.equals("new") || sAction.equals("details")){
        ...
        if(sFindProfileCode.length() > 0){
            ResultsProfile labProfile = ResultsProfile.getProfilesByProfileIDLabID(sFindProfileCode,Integer.parseInt(sProfileID));
            if(labProfile!=null){
                iTotal++;
...
<table border="0" width="100%" class="list" cellspacing="1" cellpadding="0">
  ...
  <tr>
    ...
    <td class="admin2">
        <input type="text" name="EditProfileCode" class="text" value="<%=sEditProfileCode%>" size="20" onblur="limitLength(this);">
    </td>
  </tr>
  <%-- LABEL --%>
  <%
            ...
            <tr>
              ...
              <td class="admin2">
                <table border="0" class="list" cellspacing="0" cellpadding="0">
                  ...
                  <tr class="list1">
                    <td>
                      <input type="text" name="LabCode" class="text" size="18" onBlur='blurLabAnalysis();'>
                    </td>
                    <td>
                    ...
                    <td>
                      <input type="button" class="button"  name="LabChooseButton" value="<%=getTranNoLink("Web","choose",sWebLanguage)%>" onclick="searchLabAnalysis();">&nbsp;
                      <input type="button" class="button"  name="LabAddButton"    value="<%=getTranNoLink("Web","add",sWebLanguage)%>" onClick="addLabAnalysis();">
                    </td>
                  </tr>
                  <%
                      // compose search-select
                      Vector vLabProfilesData = ResultsProfile.searchLabProfilesDataByProfileID(sProfileID);
                      ...
                      while(iter.hasNext()){
                          ...
                          %>
                              <tr class="list<%=sClass%>">
                                ...
                                %>
                                <td>
                                  <img src='<c:url value="/_img/icons/icon_delete.png"/>' border='0' alt='<%=getTranNoLink("Web","delete",sWebLanguage)%>' onclick="removeLabAnalysis('<%=sLabID%>','<%=sLabComment%>','<%=sLabCodeOther%>','<%=sLabCode%>');" onMouseOver='this.style.cursor="hand"' onMouseOut='this.style.cursor="default"'>
                                </td>
                              </tr>
                          <%
                      }
...
<script>
  ...
  function showOverview(){
    window.location.href = '<c:url value="/main.do"/>?Page=system/manageResultProfiles.jsp&ts=<%=getTs()%>';
  }
