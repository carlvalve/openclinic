			...
			for(int n=0;n<pathwayFiles.length;n++){
				...
				while(pathways.hasNext()){
					...
					if(isPathwayApplicable(pathway,signs)){
						...
						out.println("<tr><td class='mobileadmin' style='font-size:5vw;' colspan='2'>"+getLabel(pathway,sWebLanguage)+"</td></tr>");
						...
						if(pathway.element("excludes")!=null){
							...
							while(excludes.hasNext()){
								...
								if(sptSigns.get(excludeSpt)==null){
									...
									if(bMatch){
										sExcludes.append("<tr><td class='mobileadmin' style='font-size:5vw;'>"+excludeSign+"</td><td nowrap class='mobileadmin' style='font-size:5vw;'><input type='radio' id='rbspte"+pathway.attributeValue("complaint")+"_"+excludeSpt+"' name='excludeSpt."+pathway.attributeValue("complaint")+"_"+excludeSpt+"' onclick='document.getElementById(\"rbspti"+pathway.attributeValue("complaint")+"_"+excludeSpt+"\").checked=false;'> "+getTran(request,"web","excludedmini",sWebLanguage)+" <input type='radio' id='rbspti"+pathway.attributeValue("complaint")+"_"+excludeSpt+"'  onclick='document.getElementById(\"rbspte"+pathway.attributeValue("complaint")+"_"+excludeSpt+"\").checked=false;' name='includeSpt-"+pathway.attributeValue("complaint")+"_"+excludeSpt+"_"+pathway.attributeValue("complaint")+"'> "+getTran(request,"web","presentmini",sWebLanguage)+"</td></tr>");
						...
						if(bSignsToExclude){
							...
							while(childNodes.hasNext()){
								...
								if(bHasTreatments){
									out.println("<tr><td colspan='2' style='border: 1px solid black;'><table width='100%'><tr><td colspan='2'><font style='font-size:6vw;font-weight: bolder;color: grey'>"+getTran(request,"web","treatments",sWebLanguage)+"</font></td></tr>");
											...
											if(lastpart.split("\\$").length>1 && treatments.get(lastpart.split("\\$")[1])!=null){
												...
												if(treatment.sheets!=null && treatment.sheets.size()>0){
													...
													for(int q=0;q<treatment.sheets.size();q++){
														...
														if(sheet!=null){
															out.println("<td nowrap style='border: 1px solid black;'><font style='font-size:4vw;color: red'><a  style='font-size:4vw;' href='javascript:showSheet(\""+sheet.href+"\")'>"+checkString(sheet.text)+"</a></font></td>");
														}
													}
