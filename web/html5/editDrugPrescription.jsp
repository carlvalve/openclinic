...
<%
    ...
    if(activeUser==null || activeUser.person==null){
    ...
    else if(checkString(request.getParameter("formaction")).equalsIgnoreCase("delete")){
    	...
    	prescription.setServiceStockUid(checkString(prescription.getServiceStockUid()));
    	prescription.store();
    	...
    	newPrescription.setPatientUid(prescription.getPatientUid());
    	newPrescription.setPrescriberUid(prescription.getPrescriberUid());
    	...
    	prescriptionSchema = PrescriptionSchema.getPrescriptionSchema(uid);
%>
    <%=sCSSNORMAL%>
    <%=sJSPROTOTYPE%>
    <%=sJSSCRPTACULOUS%>
		...
		<div>
			<form name='transactionForm' method='post'>
				...
				<table width='100%'>
					...
					<tr>
						<td colspan='2' class='mobileadmin' style='font-size:6vw;'>
							<%=getTran(request,"web","editdrugprescription",sWebLanguage) %>
				            ...
				            if(prescription!=null && prescription.getProduct()!=null){
				            	...
				            	<tr>
					                ...
					                <td class='mobileadmin2' style='font-size: 4vw;'>
					                	<input type='text' style='width: 95% !important;font-size: 4vw;' name='drugname' id='drugname' value='<%=prescription.getProduct().getName() %>'%>
					                	<input type='hidden' name='drugid' id='drugid' value='<%=prescription.getProductUid() %>'/>
				            	...
				            	<tr>
					                ...
					                <td class='mobileadmin2' style='font-size: 4vw;'>
					                	<select name='drugunitspertimeunit' id='drugunitspertimeunit'  style='font-size: 4vw;'>
					                		<option style='font-size: 4vw;' value='0.25' <%=prescription.getUnitsPerTimeUnit()==0.5?"selected":""%>>1/4</option>
					                		<option style='font-size: 4vw;' value='0.5' <%=prescription.getUnitsPerTimeUnit()==0.5?"selected":""%>>1/2</option>
						                <%
											for(int n=1;n<10;n++){
												out.println("<option style='font-size: 4vw;' value='"+n+"' "+(n==prescription.getUnitsPerTimeUnit()?"selected":"")+">"+n+"</option>");
											}
						                %>
					                	</select>
					                	<span id='productunit' style='font-size: 4vw;'><%=getTran(request,"product.unit",prescription.getProduct().getUnit(),sWebLanguage) %> /</span>
					                	<select name='drugtimeunitcount' id='drugtimeunitcount'  style='font-size: 4vw;'>
						                <%
											for(int n=1;n<10;n++){
												out.println("<option style='font-size: 4vw;' value='"+n+"' "+(n==prescription.getTimeUnitCount()?"selected":"")+">"+n+"</option>");
											}
						                %>
					                	</select>
					                	<select name='drugtimeunit' id='drugtimeunit' style='font-size: 4vw;'>
					                		<%=ScreenHelper.writeSelectWithStyle(request, "prescription.timeunit", prescription.getTimeUnit(), sWebLanguage,"font-size: 4vw;") %>
					                	</select>
					                </td>
				            	</tr>
								<tr>
								...
								<tr>
									...
									<td class='mobileadmin2' style='font-size: 4vw;'>
										<input style='padding:4px; font-size: 4vw;' type='date' name='drugend' value='<%=prescription.getEnd()==null || prescription.getEnd().before(new java.util.Date()) ?new SimpleDateFormat("yyyy-MM-dd").format(new java.util.Date()):new SimpleDateFormat("yyyy-MM-dd").format(prescription.getEnd()) %>' size='10'/>
									</td>
								</tr>
								<tr>
									<td class='mobileadmin' style='font-size: 4vw;'>
									...
									<td class='mobileadmin2' style='font-size: 4vw;'>
								        <table>
								            <tr>
								                <td nowrap>
								                	<select name="time1" id="time1" onchange='checktime(1);' style='font-size: 4vw;'>
								                		<option/>
								                		<%
								                			for(int n=0;n<24;n++){
								                ...
								                <td nowrap>
								                	<select name="time2" id="time2" onchange='checktime(2);' style='font-size: 4vw;'>
								                		<option/>
								                		<%
								                			for(int n=0;n<24;n++){
								                ...
								                <td nowrap>
								                	<select name="time3" id="time3" onchange='checktime(3);' style='font-size: 4vw;'>
								                		<option/>
								                		<%
								                			for(int n=0;n<24;n++){
								                ...
								                <td nowrap>
								                	<select name="time4" id="time4" onchange='checktime(4);' style='font-size: 4vw;'>
								                		<option/>
								                		<%
								                			for(int n=0;n<24;n++){
								                ...
								                <td nowrap>
								                	<select name="time5" id="time5" onchange='checktime(5);' style='font-size: 4vw;'>
								                		<option/>
								                		<%
								                			for(int n=0;n<24;n++){
