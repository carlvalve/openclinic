			...
			<table width='100%'>
				<tr>
					<td colspan='2' style='font-size:8vw;text-align: right'>
						...
						<img onclick="window.location.href='getLab.jsp'" src='<%=sCONTEXTPATH%>/_img/icons/mobile/refresh.png'/>
				...
				<%
					...
					Vector transactions = MedwanQuery.getInstance().getTransactionsByType(Integer.parseInt(activePatient.personid), "be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_LAB_REQUEST");
					for(int n=0;n<transactions.size();n++){
						TransactionVO transactionVO = (TransactionVO)transactions.elementAt(n);
						out.println("<tr onclick='window.location.href=\"getLabResults.jsp?uid="+transactionVO.getServerId()+"."+transactionVO.getTransactionId()+"\";'>");
						out.println("<td class='mobileadmin2' style='font-size:6vw;'>"+ScreenHelper.formatDate(transactionVO.getUpdateTime())+"</td>");
						out.println("<td class='mobileadmin2' style='font-size:4vw;'>"+transactionVO.getUser().getPersonVO().getFullName()+"</td>");
						out.println("</tr>");
					}
				%>
			</table>
