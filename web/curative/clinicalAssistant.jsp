...
<%=checkPermission("clinicalassistant","select",activeUser)%>
<%
%>
<form name='transactionForm' method='post'>
	<table width='100%'>
		...
		%>
				<tr class='admin'>
					<td nowrap width='1%'><%=getTran(request,"web","clinicalassistant",sWebLanguage) %></td>
		...
		<%
				...
				if(contraindications.size()>0){
					...
					for(int n=0;n<contraindications.size();n++){
						...
						out.println("<tr><td nowrap><img width='32' src='"+sCONTEXTPATH+"/_img/forbidden.jpg'/> "+getTran(request,"web","contraindication",sWebLanguage)+"</td><td class='admin2'><b>"+atc.getLabel(sWebLanguage)+"</b> "+getTran(request,"web","iscontraindicatedfor",sWebLanguage)+" <b>"+MedwanQuery.getInstance().getCodeTran("icd10code"+c[1], sWebLanguage)+" ("+c[1]+")</b></td></tr>");
					}
				}
