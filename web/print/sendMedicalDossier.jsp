...
<%
    ...
    if(sAction.equals("send")){
		//Send the invoice
		%>
			<%=getTran(request,"web","sendmedicaldossier",sWebLanguage)%>...<br/>
        ...
        try{
            ...
            root.setAttributeValue("insurancenumber", insurancenumber);
           	...
           	doc.setText(HTMLEntities.xmlencode(getTranNoLink("web","transferedmedicalrecordfrom",sWebLanguage)+" "+getTranNoLink("web","hospitalname",sWebLanguage)));
...
<form name="printForm" id="printForm" method="post">
    ...
    <%=writeTableHeader("web","sendmedicaldossier",sWebLanguage)%>
    ...
    <%=ScreenHelper.alignButtonsStart()%>
        ...
        <%-- BUTTONS --%>
        <input class="button" type="button" name="printButton" value="<%=getTranNoLink("web","send",sWebLanguage)%>" onClick="doPrint();">&nbsp;
...
<script>
  ...
  function doPrint(){
	...
	if(okToPrint==true){
      printForm.Action.value = "send";
      printForm.target = "_new";
      printForm.action = "<c:url value='print/sendMedicalDossier.jsp'/>?ts=<%=getTs()%>";
      printForm.submit();
	}
  }
