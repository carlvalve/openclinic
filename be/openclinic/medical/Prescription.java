...
public class Prescription extends OC_Object{
    ...
    public void store(boolean checkExistence){
        ...
        try{
            if(this.getUid().equals("-1") && !prescrWithSameDataExists){
                ...
                ps.setString(12,ScreenHelper.checkString(this.getServiceStockUid()));
                ...
                if(!prescrWithSameDataExists){
                    ...
                    ps.setString(10,ScreenHelper.checkString(this.getServiceStockUid()));
    ...
    public boolean changed(){
        ...
        try{
            ...
            // authorization
            if(this.getAuthorization().length() > 0) ps.setString(questionmarkIdx++,this.getAuthorization());
            else                                       ps.setNull(questionmarkIdx++,Types.VARCHAR);
    ...
    public static Vector findUndelivered(String sFindPatientUid, String sFindDateBegin){
		String sSortCol="",sSortDir="";
    	Vector foundObjects = new Vector();
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try{
			// compose query
			String sSelect = "SELECT * FROM OC_PRESCRIPTIONS"+
			     " WHERE 1=1"; // difference
			
			if(sFindPatientUid.length()>0||sFindDateBegin.length()>0){
				sSelect+= " AND ";
			...
			while(rs.next()){
				Prescription p = get(rs.getString("OC_PRESCR_SERVERID")+"."+rs.getString("OC_PRESCR_OBJECTID"));
				if(p.getRequiredPackages()>p.getDeliveredQuantity()){
					foundObjects.add(p);
				}
			}
		}
    ...
    public double getDeliveredQuantity(){
        ...
        try{
            String sQuery = "select sum(OC_OPERATION_UNITSCHANGED) as total"+
                            ...
                            "   and OC_STOCK_OBJECTID=replace(OC_OPERATION_PRODUCTSTOCKUID,'"+MedwanQuery.getInstance().getConfigInt("serverId")+".','')"+
                            "   and OC_STOCK_PRODUCTUID = ?"+
                            "   and OC_OPERATION_SRCDESTTYPE = 'patient'"+
                            "   and OC_OPERATION_SRCDESTUID = ?"+
            				"   and OC_OPERATION_ENCOUNTERUID = ?";
            ps = oc_conn.prepareStatement(sQuery);
            ps.setString(1,getProductUid());
            ps.setString(2,getPatientUid());
            ps.setString(3,Encounter.getActiveEncounterOnDate(new java.sql.Timestamp(getBegin().getTime()), getPatientUid()).getUid());
            rs = ps.executeQuery();
            if(rs.next()){
    ...
    public double getExpiredDeliveredQuantity(){
        double quantity = 0;
    	Encounter encounter = Encounter.getActiveEncounterOnDate(new java.sql.Timestamp(getBegin().getTime()), getPatientUid());
    	if(encounter!=null){
	        PreparedStatement ps = null;
	        ResultSet rs = null;
	        Connection oc_conn = MedwanQuery.getInstance().getOpenclinicConnection();
	        ...
	        try{
	            String sQuery = "select sum(OC_PRESCR_REQUIREDPACKAGES) as total"+
	                            " from OC_PRESCRIPTIONS, OC_ENCOUNTERS "+
	                            "  where "+
	                            "   OC_PRESCR_PATIENTUID = OC_ENCOUNTER_PATIENTUID"+
	                            "   and OC_PRESCR_PRODUCTUID = ?"+
	                            "   and OC_PRESCR_BEGIN >= ?"+
	                            "   and OC_PRESCR_END < ?"+
	            ...
	            ps = oc_conn.prepareStatement(sQuery);
	            ps.setString(1,getProductUid());
	            ps.setDate(2,new java.sql.Date(encounter.getBegin().getTime()));
	            ps.setDate(3,new java.sql.Date(new java.util.Date().getTime()));
	            ps.setInt(4,Integer.parseInt(encounter.getUid().split("\\.")[1]));
	            rs = ps.executeQuery();
	            if(rs.next()){
	                quantity = rs.getDouble("total");
	            }
