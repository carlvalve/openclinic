            ...
            while(registers.hasNext()){
            	...
            	if(checkString(register.attributeValue("id")).equalsIgnoreCase(registerid)){
            		...
            		while(rs.next()){
            			...
            			while(columns.hasNext()){
            				org.dom4j.Element column = (org.dom4j.Element)columns.next();
            				cell = createValueCell(reg.getValue(column.attributeValue("source"), column.attributeValue("name"), column.attributeValue("translateresult")),Font.NORMAL,6,Integer.parseInt(column.attributeValue("colspan")),true);
            				table.addCell(cell);
            			}
            		}
            		rs.close();
