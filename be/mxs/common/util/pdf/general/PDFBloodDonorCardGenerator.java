...
import be.mxs.common.util.pdf.official.EndPageCardBloodBank;
 ...
 * Date: 21-nov-2006
 */
public class PDFBloodDonorCardGenerator extends PDFOfficialBasic {
    ...
    //--- CONSTRUCTOR -----------------------------------------------------------------------------
    public PDFBloodDonorCardGenerator(User user, String sProject){
        this.user = user;
    ...
    public ByteArrayOutputStream generatePDFDocumentBytes(final HttpServletRequest req, AdminPerson person) throws Exception {
        ...
        docWriter.setPageEvent(new EndPageCardBloodBank(url,contextPath,projectDir,red,green,blue));
        ...
        try {
            ...
            */
            ...
            if(MedwanQuery.getInstance().getConfigInt("enableHMKCardModel")==1){
	            ...
	            //Registration number
	            cell=createLabel(ScreenHelper.getTranNoLink("web","donornumber",user.person.language)+":",6,1,Font.ITALIC);
	            cell.setColspan(300);
	            cell.setBorder(PdfPCell.NO_BORDER);
	            cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
	            cell.setPadding(1);
            ...
            */
            String abo="?";
            String rhesus="?";
            //Find last ABO and Rhesus for the patient
        	Vector bloodgifts = MedwanQuery.getInstance().getTransactionsByType(Integer.parseInt(person.personid), "be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_CNTS_BLOODGIFT");
        	for(int n=0;n<bloodgifts.size();n++){
        		...
        		if(abo.equals("?")){
		        	...
		        	if(analysis!=null) abo=analysis.getResultValue();
        		}
        		if(rhesus.equals("?")){
        			RequestedLabAnalysis analysis = RequestedLabAnalysis.getByObjectid(bloodgift.getTransactionId(), MedwanQuery.getInstance().getConfigString("cntsRhesusCode","Rh"));
		        	if(analysis!=null) rhesus=analysis.getResultValue();
        		}
        		if(!abo.equals("?") && !rhesus.equals("?")){
        			break;
        		}
        	}
            if(abo.length()>1){
            	cell=new PdfPCell(new Paragraph(abo,FontFactory.getFont(FontFactory.HELVETICA,30,Font.BOLD)));
            }
            else{
            	cell=new PdfPCell(new Paragraph(abo,FontFactory.getFont(FontFactory.HELVETICA,60,Font.BOLD)));
            }
            cell.setBorder(PdfPCell.NO_BORDER);
            cell.setVerticalAlignment(PdfPCell.ALIGN_MIDDLE);
            ...
            cell.setPadding(0);
            table.addCell(cell);
            cell=new PdfPCell(new Paragraph(rhesus,FontFactory.getFont(FontFactory.HELVETICA,34,Font.BOLD)));
            cell.setBorder(PdfPCell.NO_BORDER);
            cell.setVerticalAlignment(PdfPCell.ALIGN_MIDDLE);
            cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
            cell.setColspan(60);
            ...
            //Expiry data
            cell=createLabel(ScreenHelper.getTranNoLink("cnts","CNTS",user.person.language),6,1,Font.ITALIC);
            cell.setColspan(1000);
            cell.setBorder(PdfPCell.NO_BORDER);
            cell.setPaddingRight(5);
            cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
