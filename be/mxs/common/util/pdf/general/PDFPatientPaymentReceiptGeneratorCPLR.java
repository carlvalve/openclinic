...
public class PDFPatientPaymentReceiptGeneratorCPLR extends PDFInvoiceGenerator {

    //--- CONSTRUCTOR -----------------------------------------------------------------------------
    public PDFPatientPaymentReceiptGeneratorCPLR(User user, AdminPerson patient, String sProject, String sPrintLanguage){
        this.user = user;
    ...
    private void printCredit(PatientCredit credit){
    	try{
	        ...
	        cell = createBorderlessCell(receiptid+" - "+ScreenHelper.getTranNoLink("web","receiptforpayment",sPrintLanguage).toUpperCase()+" "+credit.getUid().split("\\.")[1],10, 50,new Double(8*scaleFactor).intValue());
	        cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
	        cell.setPadding(2);
	        table.addCell(cell);
	        ...
	        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
	        table.addCell(cell);
	        cell = createBoldLabelCell(credit.getPatient().personid, 15,new Double(7*scaleFactor).intValue());
	        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
	        table.addCell(cell);
	        ...
	        cell = createUnderlinedTextCell(ScreenHelper.getTran(null,"web","payments",sPrintLanguage), 50*MedwanQuery.getInstance().getConfigInt("patientInvoiceReceiptCareDeliveryColumnWidthPercent",60)/100,new Double(7*scaleFactor).intValue());
	        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
	        table.addCell(cell);
	        ...
	        cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
	        table.addCell(cell);
            cell = createValueCell(ScreenHelper.getTran(null,"credit.type", credit.getType(),sPrintLanguage), 50*MedwanQuery.getInstance().getConfigInt("patientInvoiceReceiptCareDeliveryColumnWidthPercent",60)/100,new Double(7*scaleFactor).intValue(),Font.NORMAL);
            cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
            table.addCell(cell);
            ...
            cell = createValueCell(priceFormat.format(credit.getAmount())+sAlternateValue, 50-50*MedwanQuery.getInstance().getConfigInt("patientInvoiceReceiptCareDeliveryColumnWidthPercent",60)/100,new Double(7*scaleFactor).intValue(),Font.NORMAL);
            cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
            table.addCell(cell);
