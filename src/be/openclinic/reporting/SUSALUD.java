...
public class SUSALUD {
	...
	public static InConNom271 getAffiliationInformation(int personid){
		...
		inConAse.setIdRemitente(MedwanQuery.getInstance().getConfigString("susalud.ipressid","00006212")); //HEP ID
		inConAse.setIdReceptor("10001"); //SIS
		...
		inConAse.setNuRucRemitente(MedwanQuery.getInstance().getConfigString("susalud.ipressruc","20139776403"));
		inConAse.setTxRequest("CN"); //271_CON_NOM
		inConAse.setCaReceptor("2");
		inConAse.setCaPaciente("1");
		inConAse.setApPaternoPaciente("");
		inConAse.setTiDoContratante("1"); //1=DNI
		...
		try {
			...
			writer.print(x12n);
			writer.close();
			String soapResponse = sendSOAPMessage(x12n);
	...
	public static void processSOAPResponse(InConNom271 patientData, String soapResponse) throws DocumentException{
	...
	public static String sendSOAPMessage(String x12n) throws ClientProtocolException, IOException{
		Namespace soapenv = new Namespace("soapenv","http://schemas.xmlsoap.org/soap/envelope/");
		Namespace wsp = new Namespace("wsp","http://www.susalud.gob.pe/acreditacion/WSPasarelaSuSalud/");
		Namespace wsse = new Namespace("wsse","http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd");
		Namespace wsu = new Namespace("wsu","http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd");
		...
		root.addAttribute("xmlns:wsp", "http://www.susalud.gob.pe/acreditacion/WSPasarelaSuSalud/");
		
		//Create SOAP header
		Element header = root.addElement(new QName("Header",soapenv));
		Element security = header.addElement(new QName("Security",wsse));
		...
		Element req = body.addElement(new QName("getConsultaAsegCodRequest",wsp));
		Element exception = req.addElement("coExcepcion");
		exception.setText("0000");
		Element name = req.addElement("txNombre");
		name.setText("270_CON_ASE");
		Element insurer = req.addElement("coIafa");
		insurer.setText("40005"); //LA POSITIVA COMPAÑÍA DE SEGUROS Y REASEGUROS SA 40005 
		Element payload = req.addElement("txPeticion");
		payload.setText(x12n);
		
		//Send SOAP message to SUSALUD server
		String url=MedwanQuery.getInstance().getConfigString("susalud.webserviceurl","http://192.168.50.61:7800/ServicePasarela?wsdl");
