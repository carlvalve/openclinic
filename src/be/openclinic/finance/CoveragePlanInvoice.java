...
public class CoveragePlanInvoice extends Invoice {
    ...
    //--- GET -------------------------------------------------------------------------------------
    public static CoveragePlanInvoice get(String uid){
        CoveragePlanInvoice insurarInvoice = new CoveragePlanInvoice();
        ...
        if(uid!=null && uid.length()>0){
            ...
            if (ids.length==2){
                ...
                ResultSet rs = null;
                String sSelect = "SELECT * FROM OC_COVERAGEPLANINVOICES WHERE OC_INSURARINVOICE_SERVERID = ? AND OC_INSURARINVOICE_OBJECTID = ?";
                ...
                try{
                    ...
                    insurarInvoice.debets = PrestationDebet.getFullInsurarDebetsViaInvoiceUid(insurarInvoice.getUid());
    ...
    public static CoveragePlanInvoice getViaInvoiceUID(String sInvoiceID){
        CoveragePlanInvoice insurarInvoice = new CoveragePlanInvoice();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String sSelect = "SELECT * FROM OC_COVERAGEPLANINVOICES WHERE OC_INSURARINVOICE_ID = ? ";
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try{
            ps = oc_conn.prepareStatement(sSelect);
            ...
            insurarInvoice.debets = PrestationDebet.getInsurarDebetsViaInvoiceUid(insurarInvoice.getUid());
            insurarInvoice.credits = InsurarCredit.getInsurarCreditsViaInvoiceUID(insurarInvoice.getUid());
        }
        catch(Exception e){
    ...
    public boolean store(){
        ...
        try{
            if(this.getUid()!=null && this.getUid().length() > 0){
                ...
                if(ids.length == 2){
                    sSelect = "SELECT OC_INSURARINVOICE_VERSION FROM OC_COVERAGEPLANINVOICES WHERE OC_INSURARINVOICE_SERVERID = ? AND OC_INSURARINVOICE_OBJECTID = ?";
                    ps = oc_conn.prepareStatement(sSelect);
                    ps.setInt(1,Integer.parseInt(ids[0]));
                    ps.setInt(2,Integer.parseInt(ids[1]));
                    rs = ps.executeQuery();
                    ...
                    sSelect = "INSERT INTO OC_COVERAGEPLANINVOICES_HISTORY SELECT * FROM OC_COVERAGEPLANINVOICES WHERE OC_INSURARINVOICE_SERVERID = ? AND OC_INSURARINVOICE_OBJECTID = ?";
                    ps = oc_conn.prepareStatement(sSelect);
                    ps.setInt(1,Integer.parseInt(ids[0]));
                    ps.setInt(2,Integer.parseInt(ids[1]));
                    ps.executeUpdate();
                    ...
                    sSelect = " DELETE FROM OC_COVERAGEPLANINVOICES WHERE OC_INSURARINVOICE_SERVERID = ? AND OC_INSURARINVOICE_OBJECTID = ?";
                    ps = oc_conn.prepareStatement(sSelect);
                    ps.setInt(1,Integer.parseInt(ids[0]));
                    ps.setInt(2,Integer.parseInt(ids[1]));
                    ps.executeUpdate();
                    ...
                    ids = new String[] {MedwanQuery.getInstance().getConfigString("serverId"),MedwanQuery.getInstance().getOpenclinicCounter("OC_COVERAGEPLANINVOICES")+""};
            ...
            if(ids.length == 2){
               sSelect = " INSERT INTO OC_COVERAGEPLANINVOICES (" +
                ...
                sSelect = "UPDATE OC_PRESTATIONDEBETS SET OC_DEBET_INSURARINVOICEUID = NULL WHERE OC_DEBET_INSURARINVOICEUID = ? ";
                ps = oc_conn.prepareStatement(sSelect);
                ...
                if (this.debets!=null && !this.status.equalsIgnoreCase("canceled")){
                    ...
                    for (int i=0;i<this.debets.size();i++){
                        sDebetUID = ScreenHelper.checkString(((PrestationDebet)this.debets.elementAt(i)).getUid());
                        ...
                        if(counter>=250){
                            st.addBatch("UPDATE OC_PRESTATIONDEBETS SET OC_DEBET_INSURARINVOICEUID = '"+this.getUid()+"' WHERE OC_DEBET_OBJECTID in ("+UIDs+")");
                            UIDs="";
                            counter=0;
                        }
                    }
    ...
    public void updateBalance(){
    	...
    	try{
    		String sql = 	" select sum(total) total from ("+
							" select sum(oc_debet_insuraramount) total from OC_PRESTATIONDEBETS where oc_debet_insurarinvoiceuid=?"+
							" union"+ 
    		...
    		if(rs.next()){
    			...
    			sql = "update OC_COVERAGEPLANINVOICES set OC_INSURARINVOICE_BALANCE=? where OC_INSURARINVOICE_SERVERID=? and OC_INSURARINVOICE_OBJECTID=?";
    			rs.close();
    			ps.close();
    ...
    public static Vector searchInvoices(String sInvoiceDate, String sInvoiceNr, String sInvoiceInsurarUid,
        ...
        try{
            ...
            String sSql = "SELECT OC_INSURARINVOICE_SERVERID,OC_INSURARINVOICE_OBJECTID,OC_INSURARINVOICE_ID,OC_INSURARINVOICE_DATE,OC_INSURARINVOICE_INSURARUID,OC_INSURARINVOICE_CREATETIME,OC_INSURARINVOICE_UPDATETIME," +
                    "OC_INSURARINVOICE_UPDATEUID,OC_INSURARINVOICE_VERSION,OC_INSURARINVOICE_STATUS,OC_INSURARINVOICE_BALANCE FROM OC_COVERAGEPLANINVOICES WHERE ";
            ...
            while(rs.next()){
                invoice = new CoveragePlanInvoice();
    ...
    public static Vector searchInvoices(String sInvoiceDateBegin, String sInvoiceDateEnd, String sInvoiceNr, String sAmountMin, String sAmountMax){
        ...
        try{
            // compose query
            String sSql = "SELECT * FROM OC_COVERAGEPLANINVOICES WHERE ";
    ...
    public static Vector getInsurarInvoicesWhereDifferentStatus(String sStatus){
        ...
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try{
            sSelect = "SELECT * FROM OC_COVERAGEPLANINVOICES WHERE  OC_INSURARINVOICE_STATUS not in ("+sStatus+")";
            ps = oc_conn.prepareStatement(sSelect);

            rs = ps.executeQuery();
            CoveragePlanInvoice insurarInvoice;
            while(rs.next()){
                insurarInvoice = new CoveragePlanInvoice();
    ...
    public static Vector getDebetsForInvoice(String sInvoiceUid){
        ...
        try{
            sSelect = "SELECT a.lastname, a.firstname, d.*,e.* ,c.* "+
                      " FROM OC_PRESTATIONDEBETS d, OC_COVERAGEPLANINVOICES i, OC_ENCOUNTERS e, AdminView a, OC_PRESTATIONS c"+
    ...
    public static Vector getDebetsForInvoice(String sInvoiceUid,String serviceUid){
        PreparedStatement ps = null;
        ResultSet rs = null;
        ...
        try{
            sSelect = "SELECT a.lastname, a.firstname, d.*,e.* ,c.* "+
                      " FROM OC_PRESTATIONDEBETS d, OC_COVERAGEPLANINVOICES i, OC_ENCOUNTERS_VIEW e, AdminView a, OC_PRESTATIONS c"+
                      "  WHERE d.OC_DEBET_INSURARINVOICEUID = ?"+
                      "   AND i.OC_INSURARINVOICE_OBJECTID = replace(d.OC_DEBET_INSURARINVOICEUID,'"+MedwanQuery.getInstance().getConfigString("serverId")+".','')"+
                      "   AND e.OC_ENCOUNTER_OBJECTID = replace(d.OC_DEBET_ENCOUNTERUID,'"+MedwanQuery.getInstance().getConfigString("serverId")+".','')"+
                      "   AND c.OC_PRESTATION_OBJECTID = replace(d.OC_DEBET_PRESTATIONUID,'"+MedwanQuery.getInstance().getConfigString("serverId")+".','')"+
    ...
    public static Vector getDebetsForInvoiceSortByDate(String sInvoiceUid,String serviceUid){
        PreparedStatement ps = null;
        ResultSet rs = null;
    ...
    public static boolean setStatusOpen(String sInvoiceID,String UserId){
        ...
        String sSelect = "update OC_COVERAGEPLANINVOICES SET OC_INSURARINVOICE_STATUS ='open',OC_INSURARINVOICE_UPDATETIME=?,OC_INSURARINVOICE_UPDATEUID=? WHERE OC_INSURARINVOICE_OBJECTID = ? ";
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try{
            ps = oc_conn.prepareStatement(sSelect);
