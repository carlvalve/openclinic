...
public class ExporterDHIS2Activities extends Exporter {

	public void export(){
		if(!mustExport(getParam())){
			return;
		}
		if(getParam().equalsIgnoreCase("dhis2.3")){
				...
				try {
					...
					for(int n=firstYear;n<=lastYear;n++){
						...
						for(int i=firstmonth;i<=lastmonth;i++){
							...
							Connection oc_conn = MedwanQuery.getInstance().getOpenclinicConnection();
							try {
								PreparedStatement ps = oc_conn.prepareStatement("select count(*) total, oc_diagnosis_code,OC_DIAGNOSIS_SERVICEUID,c.gender,floor(datediff(oc_encounter_enddate,c.dateofbirth)/365) age,oc_encounter_type "
										...
										+ " group by OC_DIAGNOSIS_SERVICEUID,oc_diagnosis_code,c.gender,floor(datediff(oc_encounter_enddate,c.dateofbirth)/365),oc_encounter_type "
				...
				try {
					...
					for(int n=firstYear;n<=lastYear;n++){
						...
						for(int i=firstmonth;i<=lastmonth;i++){
							...
							Connection oc_conn = MedwanQuery.getInstance().getOpenclinicConnection();
							try {
								PreparedStatement ps = oc_conn.prepareStatement("select count(*) total, oc_diagnosis_code,OC_DIAGNOSIS_SERVICEUID,c.gender,floor(datediff(oc_encounter_enddate,c.dateofbirth)/365) age "
										+ " from oc_encounters a, (select distinct oc_diagnosis_encounteruid,oc_diagnosis_code,oc_diagnosis_codetype,OC_DIAGNOSIS_SERVICEUID from oc_diagnoses) b, adminview c "
										+ " where "
										+ " a.oc_encounter_objectid=replace(b.oc_diagnosis_encounteruid,'"+MedwanQuery.getInstance().getConfigInt("serverId")+".','') and"
										+ " oc_encounter_enddate >=? and oc_encounter_enddate <? and "
										...
										+ " a.oc_encounter_patientuid=c.personid"
										+ " group by OC_DIAGNOSIS_SERVICEUID,oc_diagnosis_code,c.gender,floor(datediff(oc_encounter_enddate,c.dateofbirth)/365) "
