...
public class MedwanQuery {
    ...
    public String convert(String datatype, String value){
        ...
        datatype = datatype.replaceAll("date", getConfigString("convertDataTypeDate", "date"));
    ...
    public static String translate(String term,String sourcelanguage,String targetlanguage){
    	String s = GoogleTranslate.translate(MedwanQuery.getInstance().getConfigString("googleTranslateKey","AIzaSyAPk18gciaKdwl3Z2rmFSog4ZwBbmfhByg"), sourcelanguage, targetlanguage, term);
    	return s;
    }
    ...
    public Vector findICPCCodes(String keywords, String language){
        ...
        for (int n = 0; n < codes.size(); n++){
            ...
            if(((Integer) counters.get(code)).intValue() >= mandatorykeys){
    ...
    public HealthRecordVO loadHealthRecord(PersonVO personVO, String sortOrder, SessionContainerWO sessionContainerWO){
        ...
        try{
            ...
            for (Occuprs = Occupstatement.executeQuery(); Occuprs.next();){
                ...
                updateTime = Occuprs.getTimestamp("ow_updateTime");
  ...
  //--- GET AGE ---------------------------------------------------------------------------------
    ...
    //--- GET ITEM --------------------------------------------------------------------------------
    public ItemVO getItem(int serverId, int transactionId, String itemType, String itemValue){
    	if(itemValue==null || itemValue.length()==0){
    		return getItem(serverId,transactionId,itemType);
    	}
        ItemVO itemVO = null;
        Connection OccupdbConnection;
        try{
            OccupdbConnection = getOpenclinicConnection();
            PreparedStatement Occupstatement = OccupdbConnection.prepareStatement("SELECT * from Items where serverid=? and transactionId=? and type=? and value=?");
            Occupstatement.setInt(1, serverId);
            Occupstatement.setInt(2, transactionId);
            Occupstatement.setString(3, itemType);
            Occupstatement.setString(4, itemValue);
            ResultSet Occuprs = Occupstatement.executeQuery();
            if(Occuprs.next()){
                ItemContextVO itemContextVO = new ItemContextVO(new Integer(IdentifierFactory.getInstance().getTemporaryNewIdentifier()), "", "");
                itemVO = new ItemVO(new Integer(Occuprs.getInt("itemId")), Occuprs.getString("type"), Occuprs.getString("value"), Occuprs.getDate("date"), itemContextVO);
    ...
    public boolean hasItem(String encounterUid,String itemType,String itemValue){
        ...
        try{
        	...
        	for(int n=0;n<types.length;n++){
	        	...
	        	if(values[n].length()>0){
	        		ps.setString(4, values[n].replaceAll("\\{lessthan\\}","").replaceAll("\\{morethan\\}",""));
	        	}
	        	ResultSet rs = ps.executeQuery();
	        	if(rs.next()){
	        		bHasItem=true;
    ...
    //--- GET ITEM --------------------------------------------------------------------------------
    public Vector getItemsFordPeriod(int personid, String itemTypes, java.util.Date begin, java.util.Date end){
        Vector items = new Vector();
        Connection OccupdbConnection;
        try{
            OccupdbConnection = getOpenclinicConnection();
            ...
            while(Occuprs.next()){
                ItemContextVO itemContextVO = new ItemContextVO(new Integer(IdentifierFactory.getInstance().getTemporaryNewIdentifier()), "", "");
                ItemVO itemVO = new ItemVO(new Integer(Occuprs.getInt("itemId")), Occuprs.getString("type"), Occuprs.getString("value"), Occuprs.getTimestamp("updatetime"), itemContextVO);
                items.add(itemVO);
            }
            Occuprs.close();
            Occupstatement.close();
    ...
    public ItemVO getItemHistory(int serverId, int itemId, int version){
        ItemVO itemVO = null;
        Connection OccupdbConnection;
        try{
            OccupdbConnection = getOpenclinicConnection();
            PreparedStatement Occupstatement = OccupdbConnection.prepareStatement("SELECT * from ItemsHistory where serverid=? and itemId=? and version=?");
            Occupstatement.setInt(1, serverId);
            Occupstatement.setInt(2, itemId);
            Occupstatement.setInt(3, version);
            ResultSet Occuprs = Occupstatement.executeQuery();
            if(Occuprs.next()){
                ItemContextVO itemContextVO = new ItemContextVO(new Integer(IdentifierFactory.getInstance().getTemporaryNewIdentifier()), "", "");
                itemVO = new ItemVO(new Integer(Occuprs.getInt("itemId")), Occuprs.getString("type"), Occuprs.getString("value"), Occuprs.getDate("date"), itemContextVO);
    ...
    public ItemVO getItemHistory(int serverId, int transactionId, int itemId, int version){
        ItemVO itemVO = null;
        Connection OccupdbConnection;
        try{
            OccupdbConnection = getOpenclinicConnection();
            PreparedStatement Occupstatement = OccupdbConnection.prepareStatement("SELECT * from ItemsHistory where serverid=? and transactionId=? and itemId=? and version=?");
            Occupstatement.setInt(1, serverId);
            Occupstatement.setInt(2, transactionId);
            Occupstatement.setInt(3, itemId);
            Occupstatement.setInt(4, version);
            ResultSet Occuprs = Occupstatement.executeQuery();
            if(Occuprs.next()){
                ItemContextVO itemContextVO = new ItemContextVO(new Integer(IdentifierFactory.getInstance().getTemporaryNewIdentifier()), "", "");
                itemVO = new ItemVO(new Integer(Occuprs.getInt("itemId")), Occuprs.getString("type"), Occuprs.getString("value"), Occuprs.getDate("date"), itemContextVO);
    ...
    //--- GET LAST ITEM VO (2) --------------------------------------------------------------------
    public ItemVO getLastItemVO(int personId, String itemType, java.util.Date dBefore){
        ItemVO itemVO = null;
        Connection OccupdbConnection;
        try{
            OccupdbConnection = getOpenclinicConnection();
            ...
            Occupstatement.setInt(1, personId);
            Occupstatement.setString(2, itemType);
            Occupstatement.setTimestamp(3, new java.sql.Timestamp(dBefore.getTime()));
            ResultSet Occuprs = Occupstatement.executeQuery();
            if(Occuprs.next()){
                ItemContextVO itemContextVO = new ItemContextVO(new Integer(IdentifierFactory.getInstance().getTemporaryNewIdentifier()), "", "");
                itemVO = new ItemVO(new Integer(Occuprs.getInt("itemId")), Occuprs.getString("type"), Occuprs.getString("value"), Occuprs.getDate("date"), itemContextVO);
    ...
    public PersonalVaccinationsInfoVO getPersonalVaccinationsInfo(PersonVO personVO, String language){
        ...
        try{
                ...
                try{
                    ...
                    for (elements = root.elements().iterator(); elements.hasNext();){
                        ...
                        if(element.valueOf("@ID").equals("Vaccination")){
                            ...
                            Debug.println(vaccinationInfoVO.toString());
                            break;
                        }
                    }
            ...
            String activeVaccinations = MedwanQuery.getInstance().getConfigString("activeVaccinations","*504*506*517*525*526*527*529*530*531*539*1017*1019*1020*1021*1022*1024*1025*1026*1027*");
            String id="";
            ...
            while (Occuprs.next()){
            	...
            	if(activeVaccinations.indexOf("*"+id+"*")>-1){
	                ...
	                Debug.println("other vaccination: "+Occuprs.getString("messageKey"));
            	}
            }
            Occuprs.close();
    ...
    public TransactionVO loadTransaction(String serverId, String transactionId, String version, String versionServerId){
        ...
        try{
            ...
            if(Occuprs.next()){
                ...
                transactionVO = loadTransactionHistory(serverId, transactionId, version, versionServerId);
            }
            Occuprs.close();
            Occupstatement.close();
            OccupdbConnection.close();
    ...
    public TransactionVO loadTransactionHistory(String serverId, String transactionId, String version, String versionServerId){
        ...
        try{
            ...
            if(Occuprs.next()){
                ...
                Occupstatement.setInt(3, Integer.parseInt(version));
                Occupstatement.setInt(4, Integer.parseInt(versionServerId));
                for (Occuprs = Occupstatement.executeQuery(); Occuprs.next(); transactionVO.getItems().add(getItemHistory(Integer.parseInt(serverId), Integer.parseInt(transactionId), Occuprs.getInt("itemId"),Integer.parseInt(version)))){
                }
            }
            Occuprs.close();
            Occupstatement.close();
...
//                var = "";
        ...
        try{
            ...
            if(rs.next()){
                ...
                sSelect = "insert into TransactionsHistory(transactionId,creationDate,transactionType," +
                        "  updateTime,status,healthRecordId,userId,serverid,versionserverid,version,ts)" +
                        " select transactionId,creationDate,transactionType,updateTime,status,healthRecordId," +
                        "        userId,serverid,versionserverid,version,ts" +
            ...
            else if(!transactionVO.getTransactionType().equals("be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_WORKPLACE_VISIT_AND_OTHER_ACTIVITIES")){
            ...
            ps = oc_conn.prepareStatement(sSelect);
            ps.setInt(1, transactionVO.getTransactionId().intValue());
            ps.setTimestamp(2, new java.sql.Timestamp(transactionVO.getCreationDate().getTime()));
            ps.setString(3, transactionVO.getTransactionType());
            ps.setTimestamp(4, new java.sql.Timestamp(transactionVO.getUpdateTime().getTime()));
            ...
            for(Iterator iterator = transactionVO.getItems().iterator(); iterator.hasNext();){
                ...
                if(itemVO.getValue().trim().length() > 0){
                    ...
                    ps.setString(2, itemVO.getType());
                    ps.setString(3, itemVO.getValue());
                    ps.setTimestamp(4, new java.sql.Timestamp(itemVO.getDate().getTime()));
    ...
    public FullRecordVO getHistory(int healthRecordId, String transactionType){
    	Debug.println("\n******************** getHistory ************************");
    	Debug.println("healthRecordId : "+healthRecordId);
    	Debug.println("transactionType : "+transactionType);
        ...
        try{
            OccupdbConnection = getOpenclinicConnection();
            String sQuery = "SELECT a.*,b.*,c.*,"+
                            "  a.serverid as activeServerId,"+
            		        "  a.transactionId as activeTransactionId,"+
                            "  a.healthRecordId as activeHealthrecordId"+
		                    " FROM Transactions a, Items b, ItemViewList c"+
		                    "  WHERE a.healthRecordId = ?"+
		                    "   AND a.transactionType like ?"+
		                    "   AND a.serverid = b.serverid"+
		                    "   AND a.transactionId = b.transactionId"+
		                    "   AND b.type LIKE concat('be.mxs.common.model.vo.healthrecord.IConstants.',c.itemType)"+
		                    " ORDER BY a.updateTime DESC, a.serverid, a.transactionId,"+
		                    "          c.context1, c.context2, c.context3, c.itemType";
            ...
            while(Occuprs.next()){
                ...
                Debug.println("Loaded historyItem ["+recordCounter+"] : "+checkString(Occuprs.getString("itemType")));
                recordCounter++;
    ...
    public String getUserName(int userid){
        ...
        try{
            String sQuery = "select firstname, lastname"+
    ...
    public Vector getTransactionsByEncounter(int personId, String encounterUid){
        Vector transactionList = new Vector();
        Connection OccupdbConnection;
        String sSql = "SELECT a.* FROM Transactions a, Healthrecord b, Items c"+
                      " WHERE a.healthRecordId = b.healthRecordId"+
    ...
    public static Vector getAllItemsTypesByLetters(String letters, int iMaxRows){
        ...
        // compose query
        sSelect = "SELECT "+MedwanQuery.getInstance().topFunction(""+iMaxRows)+" itemTypeId"+
                  " FROM TransactionItems"+
        		  "  where itemTypeId like '%"+letters+"%'"+
                  " group by itemTypeId"+
        		  " order by itemTypeId";
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try{
    ...
    public void setAutocompletionItemsValues(String type, String value, int user_id, int counter){
        ...
        try{
            if(!type.trim().equals("")){
                connection = getOpenclinicConnection();
                String sSql = "insert into AutoCompleteItemValues (type,value,userid,counter,itemid) values (?,?,?,?,?)";
    ...
    public void setAutocompletionCounterValues(int itemid, int counter){
        ...
        try{
            connection = getOpenclinicConnection();
            String sSql = "UPDATE AutoCompleteItemValues SET counter = ? WHERE itemid = ?";
            PreparedStatement ps = connection.prepareStatement(sSql);
            ps.setInt(1, counter);
            ps.setInt(2, itemid);
            ps.execute();
    ...
    public Vector getValuesByTypeItemByUser(String type, int userId, String letters){
        ...
        String sSelect = "SELECT itemid, value, counter"+
                         " FROM AutoCompleteItemValues"+
                         "  WHERE userid = ?"+
                         "   AND type LIKE ?" +
                         "   AND value LIKE ?"+
    ...
    public List getItemsValuesByType(String itemType){
        ...
        String sSelect = "select value from Items where type= ? GROUP BY value";
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try{
            ps = oc_conn.prepareStatement(sSelect);
    ...
    public String setSynchroniseCounters(String name, String table, String field, String bd){
        ...
        Debug.println(name+"="+iRealValue+"/"+iActualNbValue);
