...
public class PDFInsurarInvoiceGeneratorMFPSummary extends PDFInvoiceGenerator {
    ...
    //--- CONSTRUCTOR -----------------------------------------------------------------------------
    public PDFInsurarInvoiceGeneratorMFPSummary(User user, String sProject, String sPrintLanguage, String PrintType){
        this.user = user;
    ...
    public ByteArrayOutputStream generatePDFDocumentBytes(final HttpServletRequest req, String sInvoiceUid) throws Exception {
		...
		try{
            ...
            debets=new Vector();
            String[] invoices = sInvoiceUid.split(";");
            for(int n=0;n<invoices.length;n++){
	            InsurarInvoice invoice = InsurarInvoice.get(invoices[n]);
            	if(invoice!=null){
    	            Vector insurardebets = InsurarInvoice.getDebetsForInvoiceSortByDate(invoice.getUid());
    	            for(int i=0;i<insurardebets.size();i++){
    	            	Debet debet = (Debet)insurardebets.elementAt(i);
    	            	debet.setDate(invoice.getDate());
    	            	debets.add(debet);
    	            }
            		if(invoice.getDate()!=null && (start==null || invoice.getDate().before(start))){
            			start=invoice.getDate();
            		}
            		if(invoice.getDate()!=null && (end==null || invoice.getDate().after(end))){
            			end=invoice.getDate();
            		}
            	}
            }
	...
	private void addType2Invoice() throws Exception{
        model=" Mod MFP.2.A";
		SortedMap services = new TreeMap();
		SortedMap dates;
	...
	private void addType3Invoice() throws Exception{
        model=" Mod MFP.3.A";
		SortedMap services = new TreeMap();
	...
	private void addType4Invoice() throws Exception{
        model=" Mod MFP.4.A";
    ...
    private void addHeading() throws Exception {
        ...
        try {
            ...
            Report_Identification report_identification = new Report_Identification(start);
