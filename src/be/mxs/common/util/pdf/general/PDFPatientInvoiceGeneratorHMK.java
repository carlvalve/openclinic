...
public class PDFPatientInvoiceGeneratorHMK extends PDFInvoiceGenerator {
    String sProforma = "no";

    //--- CONSTRUCTOR -----------------------------------------------------------------------------
    public PDFPatientInvoiceGeneratorHMK(User user, AdminPerson patient, String sProject, String sPrintLanguage, String proforma){
        this.user = user;
    ...
    public ByteArrayOutputStream generatePDFDocumentBytes(final HttpServletRequest req, String sInvoiceUid) throws Exception {
		...
		try{
            ...
            //We will print a separate invoice for each service
            Iterator services = invoice.getServices().iterator();
            int counter=0;
            while(services.hasNext()){
            	counter++;
            	String serviceUid=(String)services.next();
                if(counter>1 || MedwanQuery.getInstance().getConfigInt("pageBreakAfterReceiptForHMK",0)==1){
                	doc.newPage();
                }
	            doc.newPage();
	            addHeading(invoice,counter);
	            addPatientData();
	            Vector debets = invoice.getDebetsForServiceUid(serviceUid);
	            addEncounterData(invoice,serviceUid,debets);
	            ...
	            for(int n=0;n<insurances.size();n++){
	                ...
	                if(insurance!=null){
	                    Vector insurancedebets=new Vector();
	                    for(int i=0;i<debets.size();i++){
	                    	Debet debet = (Debet)debets.elementAt(i);
	                    	if(ScreenHelper.checkString(debet.getInsuranceUid()).equalsIgnoreCase(insurance.getUid())){
	                    		insurancedebets.add(debet);
	                    	}
	                    }
	                    if(insurancedebets.size()>0){
	                        addInsuranceData(insurance,invoice);
	                        printDebets(invoice,insurancedebets);
	                    }
	                }
	            }
            }
    ...
    private void addEncounterData(PatientInvoice invoice,String serviceUid,Vector debets){
        try {
	    	...
	    	while(iEncounters.hasNext()){
	    		...
	    		if(encounter!=null){
	                ...
	                wrappertable.addCell(cell);
	    			cell = createGrayCell(getTran("web","providing.service").toUpperCase(), 1);
	    			wrappertable.addCell(cell);
	    			cell = createGrayCell(getTran("service",serviceUid), 1);
	    			wrappertable.addCell(cell);
	    		}
	    	}
    ...
    private void addHeading(PatientInvoice invoice,int counter) throws Exception {
        table = new PdfPTable(5);
        table.setWidthPercentage(pageWidth);
    ...
    private void printDebet(PdfPTable invoiceTable, Debet debet){
        ...
        String sEncounterName = debetEncounter.getUid()+", "+ MedwanQuery.getInstance().getLabel("encountertype", debetEncounter.getType(), sPrintLanguage)+" ("+ScreenHelper.checkString(debet.getServiceUid())+")";
