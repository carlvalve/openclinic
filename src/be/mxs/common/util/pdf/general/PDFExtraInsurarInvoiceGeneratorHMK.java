...
public class PDFExtraInsurarInvoiceGeneratorHMK extends PDFInvoiceGenerator {
    ...
    //--- CONSTRUCTOR -----------------------------------------------------------------------------
    public PDFExtraInsurarInvoiceGeneratorHMK(User user, String sProject, String sPrintLanguage, String PrintType){
        this.user = user;
    ...
    public ByteArrayOutputStream generatePDFDocumentBytes(final HttpServletRequest req, String sInvoiceUid) throws Exception {
		...
		try{
            ...
            debets = ExtraInsurarInvoice.getDebetsForInvoiceSortByDate(invoice.getUid());
	...
	private void addType1Invoice(ExtraInsurarInvoice invoice) throws Exception{
        model=" Mod MFP.1";
		...
		for(int n=0;n<debets.size();n++){
        	...
        	if(debet!=null && debet.getDate()!=null && debet.getPrestation()!=null){
            	...
            	if(invoices.get(invoiceUid)!=null){
       			...
       			income.unknown+=debet.getExtraInsurarAmount();
        ...
        while(iServices.hasNext()){
            ...
            table.addCell(cell);
            cell=createHeaderCell(ScreenHelper.getTranNoLink("web", "beneficiary", user.person.language), 70);
            cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
            cell.setBorder(PdfPCell.BOTTOM);
            table.addCell(cell);
            ...
            cell.setBorder(PdfPCell.BOTTOM);
            table.addCell(cell);
            cell=createHeaderCell(invoice.getInsurar().getName(), 10);
            cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
            cell.setBorder(PdfPCell.BOTTOM);
            table.addCell(cell);
            ...
            while(iInvoices.hasNext()){
                ...
                if(income!=null){
                	...
                	table.addCell(cell);
                	cell = createValueCell(beneficiary,70);
                    cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
                	table.addCell(cell);
	...
	private void addType2Invoice(ExtraInsurarInvoice invoice) throws Exception{
        model=" Mod MFP.2.A";
		SortedMap services = new TreeMap();
		SortedMap dates;
	...
	private void addType3Invoice(ExtraInsurarInvoice invoice) throws Exception{
        model=" Mod MFP.3.A";
		SortedMap services = new TreeMap();
		...
		for(int n=0;n<debets.size();n++){
        	...
        	if(debet!=null && debet.getDate()!=null && debet.getPrestation()!=null && !"drug".equalsIgnoreCase(ScreenHelper.checkString(debet.getPrestation().getPrestationClass()))){
        		...
        		if(diseaseType.equalsIgnoreCase("a")){
        			income.mn+=debet.getExtraInsurarAmount();
        		}
        		else if(diseaseType.equalsIgnoreCase("b")){
        			income.mp+=debet.getExtraInsurarAmount();
        		}
        		else if(diseaseType.equalsIgnoreCase("c")){
        			income.at+=debet.getExtraInsurarAmount();
        		}
        		else if(diseaseType.equalsIgnoreCase("d")){
        			income.ac+=debet.getExtraInsurarAmount();
        		}
        		else if(diseaseType.equalsIgnoreCase("e")){
        			income.aa+=debet.getExtraInsurarAmount();
        		}
        		else {
        			income.unknown+=debet.getExtraInsurarAmount();
        		}
	...
	private void addType4Invoice(ExtraInsurarInvoice invoice) throws Exception{
        model=" Mod MFP.4.A";
    ...
    private void addHeading(ExtraInsurarInvoice invoice) throws Exception {
        ...
        try {
            ...
            cell.setColspan(1);
            table2.addCell(cell);
            cell=createLabelCell(MedwanQuery.getInstance().getLabel("web.mfp","title7",user.person.language)+" "+ScreenHelper.stdDateFormat.format(new java.util.Date()), 1);
            cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
            cell.setBorder(PdfPCell.NO_BORDER);
            table2.addCell(cell);
            cell=new PdfPCell(table2);
