...
public class PDFInsurarInvoiceGeneratorASCOMA extends PDFInvoiceGenerator {
    ...
    //--- CONSTRUCTOR -----------------------------------------------------------------------------
    public PDFInsurarInvoiceGeneratorASCOMA(User user, String sProject, String sPrintLanguage, String PrintType){
        this.user = user;
    ...
    public ByteArrayOutputStream generatePDFDocumentBytes(final HttpServletRequest req, String sInvoiceUid) throws Exception {
		...
		try{
            ...
            if(hasVisits){
	            ...
	            addHeading2(invoice);
	            addInsurarData2(invoice);
	            printVisitsInvoiceDetailedInsurarOnly(invoice);
            	doc.newPage();
	            addHeading2(invoice);
	            addInsurarData2(invoice);
	            ...
	            addHeading2(invoice);
	            addInsurarData2(invoice);
	            printVisitsInvoiceDrugsDetailed2(invoice);
            }
            if(hasAdmissions){
            	if(hasVisits){
            		doc.newPage();
	            ...
	            addHeading2(invoice);
	            addInsurarData2(invoice);
	            printAdmissionInvoiceDetailed2(invoice);
            }
    ...
    protected void printAdmissionInvoiceDetailed2(InsurarInvoice invoice){
        try {
    ...
    private void addInsurarData(InsurarInvoice invoice){
        ...
        try {
            ...
            cell= createEmptyCell(50);
            table.addCell(cell);
            cell=createLabelCell(getTran("web","client"),10);
            table.addCell(cell);
            cell=createBoldLabelCell(insurar.getName(),40);
            table.addCell(cell);

            cell= createEmptyCell(60);
            table.addCell(cell);
            cell=createBoldLabelCell(insurar.getContact(),40);
            table.addCell(cell);
    ...
    protected void printVisitsInvoiceDetailed(InsurarInvoice invoice){
        try {
			...
			cell=createLabelCell("\n",100);
			table.addCell(cell);
			cell = createBoldLabelCell(getTran("web","ambulants"), 100,10);
			cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
			table.addCell(cell);
			...
			while(ePrestationClasses.hasMoreElements()){
				...
				while(eAdherents.hasNext()){
					...
					while(eBeneficiaries.hasMoreElements()){
						key=(String)eBeneficiaries.nextElement();
						cell = createBoldLabelCell(key, 30);
						cell.setBorder(PdfPCell.LEFT);
						table.addCell(cell);
						Vector debs=(Vector)beneficiaries.get(key);
						for(int n=0;n<debs.size();n++){
    ...
    protected void printVisitsInvoiceDetailedInsurarOnly(InsurarInvoice invoice){
        try {
            Vector debets = new Vector();
            for(int n=0;n<invoice.getDebets().size();n++){
            	Debet debet = (Debet)invoice.getDebets().elementAt(n);
            ...
            for(int n=0;n<debets.size();n++){
        		...
        		if(debet.getPatientInvoiceUid()!=null){
        			...
        			if(inv!=null && inv.getInsurarreference()!=null && inv.getInsurarreference().length()>0){
        				uid=inv.getInsurarreference();
        			}
        		}
            	if(bons.get(uid)==null){
            		bons.put(uid, new Hashtable());
            	}
            	beneficiaries=(Hashtable)bons.get(uid);
            	//Beneficiaries
            	if(debet.getEncounter()==null || debet.getEncounter().getPatient()==null){
            		uid="?";
            	}
            	else {
            		uid=debet.getEncounter().getPatient().getFullName();
            	}
            	if(beneficiaries.get(uid)==null){
            		beneficiaries.put(uid, new Vector());
            	}
			...
			Enumeration eBons = bons.keys();
			double generaltotal=0,bontotal=0;
			while(eBons.hasMoreElements()){
				String key = (String)eBons.nextElement();
				bontotal=0;
				cell = createBoldLabelCell(key, 100,10);
				table.addCell(cell);
				beneficiaries = (Hashtable)bons.get(key);
				Enumeration eBeneficiaries = beneficiaries.keys();
				while(eBeneficiaries.hasMoreElements()){
					key=(String)eBeneficiaries.nextElement();
					cell = createBoldLabelCell(key, 100);
					table.addCell(cell);
					Vector debs=(Vector)beneficiaries.get(key);
					for(int n=0;n<debs.size();n++){
						Debet debet = (Debet)debs.elementAt(n);
						...
						if(debet.getServiceUid()!=null){
							Service serv = Service.getService(debet.getServiceUid());
							if(serv!=null){
								service=serv.getFullyQualifiedName(sPrintLanguage);
							}
						}
						cell = createLabelCell(service, 30);
						table.addCell(cell);
						cell = createLabelCell(debet.getPrestation().getDescription(), 40);
						table.addCell(cell);
						cell = createLabelCell(new Double(debet.getInsurarAmount()).intValue()+"", 10);
						table.addCell(cell);
						bontotal+=debet.getInsurarAmount();
						generaltotal+=debet.getInsurarAmount();
					}
				}
				cell = createBoldLabelCell("", 10);
				table.addCell(cell);
				cell = createBoldLabelCell(getTran("web","subtotal"), 80);
				cell.setBorder(PdfPCell.TOP);
				table.addCell(cell);
				cell = createBoldLabelCell(new Double(bontotal).intValue()+"", 10);
				cell.setBorder(PdfPCell.TOP);
				table.addCell(cell);
				cell = createBoldLabelCell("\n", 100);
				table.addCell(cell);
    ...
    protected void printAdmissionsInvoiceDetailed(InsurarInvoice invoice){
        try {
            Vector debets = new Vector();
            for(int n=0;n<invoice.getDebets().size();n++){
            	Debet debet = (Debet)invoice.getDebets().elementAt(n);
			...
			cell=createLabelCell("\n",100);
			table.addCell(cell);
			cell = createBoldLabelCell(getTran("web","admissions").toUpperCase(), 100,10);
			cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
			table.addCell(cell);
			cell = createBoldLabelCell(getTran("web","beneficiary"), 30);
			cell.setBorder(PdfPCell.TOP+PdfPCell.LEFT+PdfPCell.RIGHT);
        ...
        try {
            ...
            for(int n=0;n<debets.size();n++){
            	...
            	if(debet.getServiceUid()==null){
            		...
            		service=debet.getServiceUid();
            	}
            	if(services.get(service)==null){
            		services.put(service, new Double(0));
            	}
            	services.put(service, (Double)services.get(service)+debet.getInsurarAmount());
            }
            cell=createBoldLabelCell(new SimpleDateFormat("MM/yyyy").format(begin)+(new SimpleDateFormat("MM/yyyy").format(begin).equalsIgnoreCase(new SimpleDateFormat("MM/yyyy").format(end))?"":" - "+new SimpleDateFormat("MM/yyyy").format(end)), 55);
            table.addCell(cell);
			cell=createLabelCell("\n",100);
			table.addCell(cell);
			cell = createBoldUnderlinedLabelCell(getTran("web","service"), 50);
			table.addCell(cell);
			cell = createBoldUnderlinedLabelCell(getTran("web","amount"), 50);
			table.addCell(cell);
			Enumeration eServices = services.keys();
			double totalamount=0;
			while(eServices.hasMoreElements()){
				String key = (String)eServices.nextElement();
				Service s = Service.getService(key);
				if(s!=null){
					Double amount = (Double)services.get(key);
					cell = createBoldLabelCell(s.getFullyQualifiedName(sPrintLanguage), 50);
					table.addCell(cell);
					cell = createBoldLabelCell(amount.intValue()+"", 50);
					table.addCell(cell);
					totalamount+=amount;
				}
			}
			cell = createBoldLabelCell(getTran("web","total"), 50);
			cell.setBorder(PdfPCell.TOP);
			table.addCell(cell);
			cell = createBoldLabelCell(new Double(totalamount).intValue()+"", 50);
			cell.setBorder(PdfPCell.TOP);
			table.addCell(cell);
			cell=createLabelCell("\n",100);
			table.addCell(cell);
