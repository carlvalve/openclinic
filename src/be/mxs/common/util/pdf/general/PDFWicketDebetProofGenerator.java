...
public class PDFWicketDebetProofGenerator extends PDFInvoiceGenerator {

    private WicketDebet wicketDebet=null;
    //--- CONSTRUCTOR -----------------------------------------------------------------------------
    public PDFWicketDebetProofGenerator(User user, WicketDebet operation, String sProject, String sPrintLanguage){
        this.user = user;
        this.wicketDebet = operation;
    ...
    public ByteArrayOutputStream generatePDFDocumentBytes(final HttpServletRequest req, String sCreditUid) throws Exception {
		...
		try{
            ...
            addHeading(wicketDebet);
            printCreditReceipt(wicketDebet);
        }
		catch(Exception e){
			baosPDF.reset();
			e.printStackTrace();
    ...
    private void addHeading(WicketDebet credit) throws Exception {
        ...
        try {
            ...
            //*** title ***
            table.addCell(createTitleCell(getTran("web","wicketdebetproof").toUpperCase()+" #"+credit.getUid().split("\\.")[1]+" - "+ScreenHelper.stdDateFormat.format(credit.getOperationDate()),"",4));
    ...
    //--- PRINT INVOICE ---------------------------------------------------------------------------
    private void printCreditReceipt(WicketDebet credit){
        try {
            PdfPTable table = new PdfPTable(1);
            table.setWidthPercentage(pageWidth);
            ...
            creditTable.addCell(createEmptyCell(100));
            creditTable.addCell(createEmptyCell(100));
            creditTable.addCell(createGrayCell(getTran("web","benificiary.signature").toUpperCase(),100));
            creditTable.addCell(createEmptyCell(100));
            creditTable.addCell(createEmptyCell(100));
            creditTable.addCell(createEmptyCell(100));
            creditTable.addCell(createEmptyCell(100));
