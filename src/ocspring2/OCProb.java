...
public class OCProb {
    ...
    public void setupProb() throws SQLException {
    	double p2 = Db.get_float("select sum(P_AFRICA) from diag where P_AFRICA<>0");
    	String s2 = String.valueOf(p2);
    	mysql = "insert into probabilita(sessionid,NRZ,PRO1) SELECT  '"+sessionid+"',NRZ,log10((P_AFRICA / " + s2 + ") / (1 - (P_AFRICA / " + s2 + "))) FROM diag d WHERE P_AFRICA  <> 0 ";
    	Db.runsql(mysql);
    }   
